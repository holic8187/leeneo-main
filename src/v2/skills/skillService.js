...
const ACTIVE_BUFF_EFFECT_KEYS = Object.freeze([
  'attackIncrease', 'magicAttackIncrease', 'defenseIncrease', 'magicDefenseIncrease',
  ...
  'magicMpCostIncreasePercent', 'mpCostReductionPercent', 'stealth',
  'physicalDamageReductionPercent', 'fireDamageReductionPercent',
  'poisonDamageReductionPercent', 'iceDamageReductionPercent',
  'lightningDamageReductionPercent', 'holyDamageReductionPercent',
  'onHitAccuracyDebuffChance', 'onHitAccuracyDebuffPercent',
  'onHitMovementSlowChance', 'onHitMovementSlowAmount', 'onHitDebuffSeconds',
  'componentDropChance', 'moneyGuardPercent', 'moneyGuardEfficiencyPercent',
  'damageImmunity',
  'statusImmunity', 'magicReflectChance', 'magicReflectPercent',
  'magicReflectCapPercent', 'hpDrain', 'hpDrainIntervalSeconds'
]);
...
function ensureSkillState(character) {
  ...
  skills.explosiveComponents = Math.max(
    0,
    Math.floor(Number(skills.explosiveComponents) || 0)
  );
...
function resolveSkillCastProfile(values = {}) {
  ...
  const postCastDelaySeconds = Math.max(0, Number(values.postCastDelaySeconds) || 0);
...
function getActiveSkillEffects(character, now = Date.now()) {
  ...
  const effects = {
    ...
    poisonDamageReductionPercent: 0,
    iceDamageReductionPercent: 0,
    lightningDamageReductionPercent: 0,
    holyDamageReductionPercent: 0,
    ...
    moneyGuardPercent: 0,
    moneyGuardEfficiencyPercent: 0,
    damageImmunity: 0,
    statusImmunity: 0,
    magicReflectChance: 0,
    magicReflectPercent: 0,
    magicReflectCapPercent: 0,
    hpDrain: 0,
    hpDrainIntervalSeconds: 0,
    throwingStarCapacityIncrease: 0,
    blockInvincibleSeconds: 0,
    periodicHpRestore: 0,
    periodicMpRestore: 0,
  ...
  for (const definition of getDepartmentSkillDefinitions(character.job?.departmentId)) {
    ...
    if (definition.effect === 'weapon-mastery' && definition.weaponTypes.includes(weaponType)) {
      ...
      effects.attackIncrease += Number(values.attackIncrease) || 0;
      effects.throwingStarCapacityIncrease = Math.max(
        effects.throwingStarCapacityIncrease,
        Number(values.throwingStarCapacityIncrease) || 0
      );
    }
    ...
    if (definition.effect === 'damage-reduction') {
      const reduction = Number(values.reductionPercent) || 0;
      const physicalReduction = Number(values.physicalDamageReductionPercent) || 0;
      const elements = Array.isArray(values.damageReductionElements)
        ? values.damageReductionElements
        : [];
      if (physicalReduction > 0) {
        effects.physicalDamageReductionPercent += physicalReduction;
      } else if (elements.length) {
        for (const element of elements) {
          const key = `${String(element)}DamageReductionPercent`;
          if (Object.prototype.hasOwnProperty.call(effects, key)) effects[key] += reduction;
        }
      } else {
        effects.damageReductionPercent += reduction;
      }
    }
    ...
    if (definition.effect === 'shield-block' && character.loadout?.shield) {
      ...
      effects.blockInvincibleSeconds = Math.max(
        effects.blockInvincibleSeconds,
        Number(values.invincibleSeconds) || 0
      );
    }
