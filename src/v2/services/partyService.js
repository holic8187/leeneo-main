...
function hasRecentPartyMonsterKill(userId, now = Date.now()) {
  const party = getParty(userId);
  if (!party) return false;
  const member = party.members.find((entry) => entry.userId === String(userId));
  const lastKillAt = Math.max(0, Number(member?.lastKillAt) || 0);
  return lastKillAt > 0 && Number(now) - lastKillAt <= PARTY_EXP_ACTIVITY_WINDOW_MS;
}
