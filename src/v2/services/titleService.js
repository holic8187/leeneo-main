...
const TITLE_DEFINITIONS = Object.freeze([
  Object.freeze({
    ...
    condition: '레벨 10 이상이며 전직 완료(차수 무관)',
...
function reconcileAdvancementTitles(character) {
  ...
  const level = Math.max(1, Number(character.progression?.level) || 1);
  const departmentId = String(character.job?.departmentId || '').trim();
  const hasAssignedJob = tier >= 1 || Boolean(departmentId && departmentId !== 'unassigned');
  const awarded = [];
  if (level >= 10 && hasAssignedJob) {
