...
function syncCharacterBusStopLocation(character, mapId, x = 12) {
  const requestedMapId = String(mapId || '');
  if (!isBusStopMap(requestedMapId)) {
    return String(character?.worldState?.mapId || '');
  }
  setBusMap(character, requestedMapId, Math.max(0, Math.min(94, Number(x) || 12)));
