...
const { rollSideJobWeaponInstanceData } = require('../items/equipmentCatalog');
const {
  addInventoryItem,
...
const SIDE_JOB_SUCCESS_RATE = 0.9;
...
const SIDE_JOB_RECIPES = Object.freeze([
  ...
  ['crafted_infinite_overtime_crossbow', 'monster_loot_peach_gate_sentry', 'monster_loot_server_wisp'],
...
function craftSideJobWeapon(character, weaponId, random = Math.random, now = Date.now()) {
  ...
  if (success) {
    instanceData = rollSideJobWeaponInstanceData(weapon, random);
