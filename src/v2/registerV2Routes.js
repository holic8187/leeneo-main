...
function registerV2Routes({
  ...
  function buildBossExpeditionView(userId, profile = null) {
    ...
    for (const expiredUserId of state.expiredUserIds || []) {
      removeBossExpeditionMember(expiredUserId, 'gammam_neo');
      removeBossExpeditionMember(expiredUserId, 'mad_hwang_manager');
    }
  ...
  app.post('/api/v2/boss-expeditions/slot', async (req, res) => {
    ...
    try {
      ...
      const definition = before.definitions.find((entry) => entry.id === bossId);
      if (!unregistering) {
        const entryBlock = getFieldBossEntryBlock(profile, getWorldMap(definition?.mapId));
        if (entryBlock) throw new Error('감맘 네오 처치 후 24시간 동안 다시 입장할 수 없습니다.');
      }
  ...
  app.post('/api/v2/boss-expeditions/respond', async (req, res) => {
    ...
    try {
      ...
      if (result.ready) {
        ...
        return res.json({ success: true, entered: true, ...entry });
      }
      const profile = await getWorldProfile(auth.id);
      return res.json({
        success: true,
