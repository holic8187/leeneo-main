...
function registerV2Routes({
  ...
  function hasEnteredRaidToday(character, bossId) {
    return String(character?.bossRaidEntries?.[String(bossId || '')] || '') === getKoreaDateKey();
  }
  ...
  async function applyRaidBossRewardEvent(event) {
    ...
    for (const share of phaseShares) {
      await withCharacterMutation(share.userId, async () => {
        const character = await V2Character.findOne({ userId: share.userId });
        if (!character) return;
    ...
    for (const character of allCharacters) {
      ...
      await character.save();
      worldProfileCache.delete(userId);
    }
    ...
    try {
        ...
        await Promise.all(state.contactEvents.map((event) => (
          withCharacterMutation(event.userId, async () => {
            ...
            if (augmentProtection) {
              ...
              const requiredExp = getRequiredExpV2(character.progression?.level);
              const currentExp = Math.max(0, Number(character.progression?.exp) || 0);
              const isRaidDeath = Boolean(getWorldMap(state.mapId)?.raidBossId);
              event.expLost = Math.min(
                currentExp,
                Math.floor(requiredExp * (isRaidDeath ? 0.01 : 0.1))
              );
      ...
      const partyMembers = (partyState.party?.members || []).map((member) => {
        ...
        return {
          ...
          currentHp: presence?.currentHp ?? (isSelf ? profile.resources.currentHp : 0),
          maxHp: presence?.maxHp ?? (isSelf ? profile.resources.maxHp : 1),
  ...
  app.post('/api/v2/boss-expeditions/slot', async (req, res) => {
    ...
    try {
      ...
      if (!unregistering) {
        const character = await V2Character.findOne({ userId: auth.id }).select('bossRaidEntries');
        if (hasEnteredRaidToday(character, bossId)) {
  ...
  app.post('/api/v2/boss-expeditions/start', async (req, res) => {
    ...
    try {
      ...
      const entryRecords = await V2Character.find({ userId: { $in: memberIds } })
        .select('userId bossRaidEntries worldState.mapId');
      if (entryRecords.some((character) => hasEnteredRaidToday(character, bossId))) {
  ...
  app.post('/api/v2/boss-expeditions/respond', async (req, res) => {
    ...
    try {
      ...
      if (result.ready) {
        ...
        if (characters.some((character) => hasEnteredRaidToday(character, bossId))) {
          ...
          await V2Character.updateOne(
            { userId },
            {
              $set: {
                ...
                'worldState.raidDeadAt': null,
                [`bossRaidEntries.${bossId}`]: getKoreaDateKey(),
                'huntingTime.enabled': false,
                'huntingTime.lastTickAt': null
              }
            }
