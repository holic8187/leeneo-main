...
function registerV2Routes({
  ...
  async function persistCharacterSnapshot(key, snapshot, mailboxEntries = []) {
    let lastError = null;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        const mutableFields = { ...snapshot };
        delete mutableFields._id;
        delete mutableFields.__v;
        delete mutableFields.userId;
        delete mutableFields.mailbox;
        delete mutableFields.createdAt;
        delete mutableFields.updatedAt;
  ...
  async function drainCharacterPersistence(key, state) {
    try {
      while (state.latestSnapshot) {
        ...
        try {
          await persistCharacterSnapshot(key, snapshot, mailboxEntries);
        } catch (error) {
  ...
  function queueCharacterPersistence(character, { syncMailbox = false } = {}) {
