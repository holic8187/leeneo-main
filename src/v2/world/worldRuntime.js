...
const ONLINE_SESSION_TIMEOUT_MS = 90_000;
...
function getActiveMonsterDebuffPercent(monster, valueKey, expiryKey, now = Date.now()) {
  if (Number(monster?.[expiryKey] || 0) <= now) return 0;
  return Math.max(0, Math.min(95, Number(monster?.[valueKey]) || 0));
}

function applyMonsterIncomingDamageIncrease(monster, damage, now = Date.now()) {
  const increasePercent = getActiveMonsterDebuffPercent(
    monster,
    'incomingDamageIncreasePercent',
    'incomingDamageDebuffUntil',
    now
  );
  return Math.max(0, Math.floor(Number(damage || 0) * (1 + increasePercent / 100)));
}

function getEffectiveMonsterDefense(monster, damageType, now = Date.now()) {
  const baseDefense = Math.max(
    0,
    Number(damageType === 'magic' ? monster?.magicDefense : monster?.physicalDefense) || 0
  );
  const defenseIncrease = Number(monster?.defenseModifierUntil || 0) > now
    ? Math.max(0, Number(monster?.defenseIncreasePercent) || 0)
    : 0;
  const physicalReduction = damageType !== 'magic'
    && Number(monster?.physicalDefenseReductionUntil || 0) > now
    ? Math.max(0, Math.min(100, Number(monster?.physicalDefenseReductionPercent) || 0))
    : 0;
  return Math.max(0, baseDefense * (1 + defenseIncrease / 100) * (1 - physicalReduction / 100));
}

function getSkillElementMultiplier(monster, elements, now = Date.now()) {
  const activeElements = Array.isArray(elements) && elements.length ? elements : ['neutral'];
  return Math.max(...activeElements.map((activeElement) => {
    const baseMultiplier = getElementMultiplier(monster, activeElement);
    const weaknessMultiplier = Number(monster?.temporaryWeaknessUntil || 0) > now
      && String(monster?.temporaryWeaknessElement || '') === String(activeElement)
      ? Math.max(1, Number(monster?.temporaryWeaknessMultiplier) || 1)
      : 1;
    return baseMultiplier * weaknessMultiplier;
  }));
}

function getPlayerDamageReduction(player, damageType = 'physical', element = 'neutral') {
  const profile = player?.combatProfile || {};
  const globalReduction = Math.max(0, Number(profile.damageReductionPercent) || 0);
  const typeReduction = damageType === 'physical'
    ? Math.max(0, Number(profile.physicalDamageReductionPercent) || 0)
    : 0;
  const elementKey = `${String(element || 'neutral')}DamageReductionPercent`;
  const elementReduction = Math.max(0, Number(profile[elementKey]) || 0);
  return Math.max(0, Math.min(95, globalReduction + typeReduction + elementReduction));
}
...
function createMonster(map, index, now, platform = null, speciesOverride = null) {
  ...
  return {
    ...
    incomingDamageIncreasePercent: 0,
    incomingDamageDebuffUntil: 0,
    monsterAccuracyReductionPercent: 0,
    monsterAccuracyDebuffUntil: 0,
    combatDebuffSkillId: '',
    combatDebuffName: '',
    combatDebuffUntil: 0,
    movementSpeedReduction: 0,
    movementSpeedDebuffUntil: 0,
    skillSealedUntil: 0,
    physicalDefenseReductionPercent: 0,
    physicalDefenseReductionUntil: 0,
    defenseIncreasePercent: 0,
    defenseModifierUntil: 0,
    experienceIncreasePercent: 0,
    dropRateIncreasePercent: 0,
    rewardDebuffUntil: 0,
    temporaryWeaknessElement: '',
    temporaryWeaknessMultiplier: 1,
    temporaryWeaknessUntil: 0,
    skillDotStacks: [],
    droppedComponentsByUser: {},
    additionalMiscGrantedByUser: {},
    aggroTargetId: '',
    counterAttackTargetId: '',
    counterAttackReadyAt: 0,
    counterAttackCooldownUntil: 0
  };
}
...
function serializeMonster(monster) {
  return {
    ...
    incomingDamageIncreasePercent: Math.max(
      0,
      Number(monster.incomingDamageIncreasePercent) || 0
    ),
    incomingDamageDebuffUntil: Math.max(0, Number(monster.incomingDamageDebuffUntil) || 0),
    monsterAccuracyReductionPercent: Math.max(
      0,
      Number(monster.monsterAccuracyReductionPercent) || 0
    ),
    monsterAccuracyDebuffUntil: Math.max(0, Number(monster.monsterAccuracyDebuffUntil) || 0),
    combatDebuffSkillId: String(monster.combatDebuffSkillId || ''),
    combatDebuffName: String(monster.combatDebuffName || ''),
    combatDebuffUntil: Math.max(0, Number(monster.combatDebuffUntil) || 0),
    movementSpeedReduction: Math.max(0, Number(monster.movementSpeedReduction) || 0),
    movementSpeedDebuffUntil: Math.max(0, Number(monster.movementSpeedDebuffUntil) || 0),
    skillSealedUntil: Math.max(0, Number(monster.skillSealedUntil) || 0),
    physicalDefenseReductionPercent: Math.max(
      0,
      Number(monster.physicalDefenseReductionPercent) || 0
    ),
    physicalDefenseReductionUntil: Math.max(
      0,
      Number(monster.physicalDefenseReductionUntil) || 0
    ),
    defenseIncreasePercent: Math.max(0, Number(monster.defenseIncreasePercent) || 0),
    defenseModifierUntil: Math.max(0, Number(monster.defenseModifierUntil) || 0),
    temporaryWeaknessElement: String(monster.temporaryWeaknessElement || ''),
    temporaryWeaknessMultiplier: Math.max(1, Number(monster.temporaryWeaknessMultiplier) || 1),
    temporaryWeaknessUntil: Math.max(0, Number(monster.temporaryWeaknessUntil) || 0),
    dotActive: Array.isArray(monster.skillDotStacks) && monster.skillDotStacks.length > 0,
    ...
    counterAttackType: String(monster.counterAttackType || ''),
    counterAttackReadyAt: Math.max(0, Number(monster.counterAttackReadyAt) || 0),
...
function serializePlayer(player, now = Date.now()) {
  ...
  return {
    ...
    equippedTitle: player.equippedTitle ? { ...player.equippedTitle } : null,
    ...
    online: isOnlineSessionActive(player.userId, now)
      || now - Number(player.lastSeenAt || 0) <= PLAYER_TIMEOUT_MS,
...
function findNearestDecoy(runtime, monster, now = Date.now()) {
  return findNearestDecoyFromCandidates(getActiveDecoys(runtime, now), monster);
}

function findNearestDecoyFromCandidates(candidates, monster) {
  return (candidates || [])
...
function advanceMonster(monster, runtime, map, deltaSeconds, now) {
  ...
  const movementSlow = Number(monster.movementSpeedDebuffUntil || 0) > now
    ? Math.max(0, Math.min(100, Number(monster.movementSpeedReduction) || 0))
    : 0;
  const effectiveMovementSpeed = Math.max(
    0,
    Number(monster.movementSpeed || 0) * (1 - movementSlow / 100)
  );
  if (target && target.floor === monster.floor) {
    const difference = target.x - monster.x;
    const counterAttackRangePercent = Number(monster.counterAttackReadyAt) > 0
      ? Number(monster.counterAttackRangePx) / worldWidth * 100
      : 0;
    const stoppingDistance = counterAttackRangePercent > 0
      ? Math.max(2.2, counterAttackRangePercent * 0.78)
      : 2.2;
    if (Math.abs(difference) > stoppingDistance) {
      monster.direction = difference < 0 ? -1 : 1;
      monster.state = 'chase';
      const step = effectiveMovementSpeed / worldWidth * 100 * deltaSeconds;
...
function armMonsterCounterAttack(monster, userId, now = Date.now()) {
  if (
    !monster
    || monster.fieldBoss
    || monster.raidBoss
    ...
    || !monster.counterAttackType
  ) return false;
  const userKey = String(userId || '');
  if (!userKey) return false;
  monster.counterAttackTargetId = userKey;
  monster.aggroTargetId = userKey;
  if (
    Number(monster.counterAttackReadyAt) > 0
    || Number(monster.counterAttackCooldownUntil) > now
  ) return false;
  monster.counterAttackReadyAt = now + Math.max(
    250,
    Number(monster.counterAttackWindupMs) || 600
  );
  monster.state = 'attack-windup';
  return true;
}

function rollMonsterCounterAttackDamage(monster) {
  const minimum = Math.max(1, Math.floor(Number(monster.counterAttackMinDamage) || 1));
  const maximum = Math.max(minimum, Math.floor(Number(monster.counterAttackMaxDamage) || minimum));
  return minimum + Math.floor(Math.random() * (maximum - minimum + 1));
}

function getEffectivePhysicalDodgeChance(player, monster, now = Date.now()) {
  const profile = player?.combatProfile || {};
  const statDodgeChance = Number(profile.evasion) > 0
    ? calculateIncomingPhysicalEvadeChance({
    ...
    : 0;
  const passiveDodgeChance = Math.max(
    0,
    Math.min(100, Number(profile.dodgeChance) || 0)
  );
  const accuracyReductionPercent = getActiveMonsterDebuffPercent(
    monster,
    'monsterAccuracyReductionPercent',
    'monsterAccuracyDebuffUntil',
    now
  );
  return 100 - (
    (100 - statDodgeChance)
    * (1 - passiveDodgeChance / 100)
    * (1 - accuracyReductionPercent / 100)
  );
}

function applyMonsterCounterAttacks(runtime, now, protectedDecoys = getActiveDecoys(runtime, now)) {
  const events = [];
  const worldWidth = getWorldWidth(runtime.mapId);
  for (const monster of runtime.monsters) {
    if (
      monster.hp <= 0
      || monster.fieldBoss
      || Number(monster.counterAttackReadyAt) <= 0
      || now < Number(monster.counterAttackReadyAt)
    ) continue;
    const player = runtime.players.get(String(monster.counterAttackTargetId || ''));
    if (!player || player.currentHp <= 0 || Number(player.combatProfile?.stealth) > 0) {
      monster.counterAttackReadyAt = 0;
      monster.counterAttackTargetId = '';
      continue;
    }
    const decoy = findNearestDecoyFromCandidates(protectedDecoys, monster);
    const target = decoy || player;
    const rangePercent = Math.max(1, Number(monster.counterAttackRangePx) || 180)
      / worldWidth * 100;
    if (
      Number(target.floor) !== Number(monster.floor)
      || Math.abs(Number(target.x) - Number(monster.x)) > rangePercent + 4.5
    ) {
      monster.state = 'chase';
      continue;
    }

    monster.direction = Number(target.x) < Number(monster.x) ? -1 : 1;
    monster.state = 'attacking';
    monster.counterAttackReadyAt = 0;
    monster.counterAttackCooldownUntil = now + Math.max(
      1_000,
      Number(monster.counterAttackCooldownMs) || 3_500
    );
    monster.lastCounterAttackAt = now;
    const outgoingReduction = now < Number(monster.outgoingDamageDebuffUntil || 0)
      ...
      : 0;
    const rolledCounterDamage = rollMonsterCounterAttackDamage(monster)
      * (1 - outgoingReduction / 100);
    if (decoy) {
      damageDecoy(
        runtime,
        decoy,
        monster,
        rolledCounterDamage,
        monster.counterAttackType === 'projectile'
          ? 'monster-projectile'
          : 'monster-swing',
        now
      );
    ...
    if (now < Number(player.invulnerableUntil || 0)) continue;

    const calculation = calculateIncomingPhysicalDamage({
      monsterAttack: rolledCounterDamage,
    ...
    const effectiveDodgeChance = getEffectivePhysicalDodgeChance(player, monster, now);
    const dodged = Math.random() * 100 < effectiveDodgeChance;
    const blocked = !dodged
      && Math.random() * 100 < Number(player.combatProfile.blockChance || 0);
    const reduction = getPlayerDamageReduction(player, 'physical', 'neutral');
    const damageTakenIncrease = Math.max(
      0,
      Number(player.combatProfile.damageTakenIncreasePercent) || 0
    );
    let incomingDamage = blocked || dodged || Number(player.combatProfile.damageImmunity) > 0
      ? 0
    ...
    let moneySpent = 0;
    const guardPercent = Math.max(
      0,
      Math.min(100, Number(player.combatProfile.moneyGuardPercent) || 0)
    );
    const guardEfficiency = Math.max(
      1,
      Number(player.combatProfile.moneyGuardEfficiencyPercent) || 100
    );
    if (incomingDamage > 0 && guardPercent > 0 && Number(player.combatProfile.currentMoney) > 0) {
      const desiredPrevention = Math.floor(incomingDamage * guardPercent / 100);
      const moneyCostPerDamage = guardEfficiency / 100;
      const affordablePrevention = Math.floor(
        Number(player.combatProfile.currentMoney) / moneyCostPerDamage
      );
      const prevented = Math.max(0, Math.min(desiredPrevention, affordablePrevention));
      moneySpent = Math.ceil(prevented * moneyCostPerDamage);
      incomingDamage = Math.max(0, incomingDamage - prevented);
      player.combatProfile.currentMoney = Math.max(
        0,
        Number(player.combatProfile.currentMoney) - moneySpent
      );
    }
    ...
    player.invulnerableUntil = now + (
      blocked && Number(player.combatProfile.blockInvincibleSeconds) > 0
        ? Number(player.combatProfile.blockInvincibleSeconds) * 1000
        : CONTACT_INVULNERABILITY_MS
    );
    events.push({
      userId: player.userId,
      source: monster.counterAttackType === 'projectile'
        ? 'monster-projectile'
        : 'monster-swing',
      monsterName: monster.name,
      monsterAttackType: monster.counterAttackType,
...
function applyContactDamage(runtime, now, protectedDecoys = getActiveDecoys(runtime, now)) {
  ...
  for (const monster of runtime.monsters) {
    if (monster.hp <= 0 || Number(monster.lastCounterAttackAt) === now) continue;
    const decoy = findNearestDecoyFromCandidates(protectedDecoys, monster);
  ...
  for (const player of runtime.players.values()) {
    ...
    const collider = runtime.monsters.find((monster) => {
      if (
        ...
        || Number(monster.lastCounterAttackAt) === now
    ...
    const effectiveDodgeChance = getEffectivePhysicalDodgeChance(player, collider, now);
    const dodged = Math.random() * 100 < effectiveDodgeChance;
    const blocked = !dodged
      && Math.random() * 100 < Number(player.combatProfile.blockChance || 0);
    const reduction = getPlayerDamageReduction(player, 'physical', 'neutral');
    ...
    let moneySpent = 0;
    const guardPercent = Math.max(0, Math.min(100, Number(player.combatProfile.moneyGuardPercent) || 0));
    const guardEfficiency = Math.max(1, Number(player.combatProfile.moneyGuardEfficiencyPercent) || 100);
    if (incomingDamage > 0 && guardPercent > 0 && Number(player.combatProfile.currentMoney) > 0) {
      const desiredPrevention = Math.floor(incomingDamage * guardPercent / 100);
      const moneyCostPerDamage = guardEfficiency / 100;
      const affordablePrevention = Math.floor(
      ...
      moneySpent = Math.ceil(prevented * moneyCostPerDamage);
      incomingDamage = Math.max(0, incomingDamage - prevented);
      player.combatProfile.currentMoney = Math.max(0, Number(player.combatProfile.currentMoney) - moneySpent);
    }
    const damageSplit = splitDamageWithMpGuard(incomingDamage, {
      currentMp: player.currentMp,
      guardPercent: player.combatProfile.mpDamageGuardPercent
    ...
    damagedPlayers.push({
      userId: player.userId,
      source: collider.raidBoss
        ? 'raid-boss-contact'
        : (collider.fieldBoss ? 'field-boss-contact' : 'monster-contact'),
      monsterName: collider.name,
      damage: damageSplit.hpDamage,
      totalDamage: damageSplit.totalDamage,
      mpDamage: damageSplit.mpDamage,
      blocked,
...
function resistsStatusEffect(player, random = Math.random) {
  if (Number(player?.combatProfile?.statusImmunity) > 0) return true;
  const resistance = Math.max(
    0,
    Math.min(100, Number(player?.combatProfile?.statusResistance) || 0)
  );
  return resistance > 0 && Number(random()) * 100 < resistance;
}

function applyFixedFieldBossDamage(
  player,
  boss,
  baseDamage,
  source,
  now,
  { damageType = 'physical', element = 'neutral' } = {}
) {
  if (!player || now < Number(player.invulnerableUntil || 0)) return null;
  const dodgeChance = damageType === 'physical'
    ? getEffectivePhysicalDodgeChance(player, boss, now)
    : Number(player.combatProfile?.dodgeChance || 0);
  const dodged = Math.random() * 100 < dodgeChance;
  const reduction = getPlayerDamageReduction(player, damageType, element);
  const damageTakenIncrease = Math.max(
    0,
    Number(player.combatProfile?.damageTakenIncreasePercent) || 0
  );
  const outgoingReduction = Number(boss?.outgoingDamageDebuffUntil || 0) > now
    ? Math.max(0, Math.min(95, Number(boss?.outgoingDamageReductionPercent) || 0))
    : 0;
  const damage = dodged || Number(player.combatProfile?.damageImmunity) > 0
    ? 0
    : Math.max(1, Math.floor(
      (Number(baseDamage) || 1) * (1 - outgoingReduction / 100)
        * (1 - reduction / 100)
        * (1 + damageTakenIncrease / 100)
    ));
  ...
  if (
    damageType === 'magic'
    && !dodged
    && damageSplit.totalDamage > 0
    && Math.random() * 100 < Number(player.combatProfile?.magicReflectChance || 0)
  ) {
    const cap = Math.max(0, Number(boss?.maxHp) || 0)
      * Math.max(0, Number(player.combatProfile?.magicReflectCapPercent) || 0) / 100;
    reflectedDamage = Math.max(0, Math.floor(Math.min(
      damageSplit.totalDamage * Number(player.combatProfile?.magicReflectPercent || 0) / 100,
      cap
    )));
    if (reflectedDamage > 0) {
      recordMonsterContribution(boss, player.userId, reflectedDamage);
      boss.hp = Math.max(boss.fieldBoss ? 1 : 0, Number(boss.hp) - reflectedDamage);
    }
  }
  return {
    userId: player.userId,
...
function applyRaidResourceCrash(player, boss, now = Date.now()) {
  if (!player || now < Number(player.invulnerableUntil || 0)) return null;
  const dodged = Math.random() * 100 < getEffectivePhysicalDodgeChance(player, boss, now);
  const previousHp = Math.max(1, Number(player.currentHp) || 1);
  const previousMp = Math.max(0, Number(player.currentMp) || 0);
  if (!dodged && !(Number(player.combatProfile?.damageImmunity) > 0)) {
    player.currentHp = 1;
    player.currentMp = 1;
  }
  player.invulnerableUntil = now + CONTACT_INVULNERABILITY_MS;
  return {
    userId: player.userId,
    damage: dodged || Number(player.combatProfile?.damageImmunity) > 0
      ? 0 : Math.max(0, previousHp - 1),
    totalDamage: dodged || Number(player.combatProfile?.damageImmunity) > 0
      ? 0 : Math.max(0, previousHp - 1),
    mpDamage: dodged || Number(player.combatProfile?.damageImmunity) > 0
      ? 0 : Math.max(0, previousMp - 1),
    blocked: false,
    dodged,
    resistedKnockback: true,
    reflectedDamage: 0,
...
function createBaldKimSummons(runtime, boss, definition, now) {
  ...
  const activeSummonCount = runtime.monsters.filter((monster) => (
    monster.hp > 0 && String(monster.summonedByRaidBoss || '') === String(boss.id)
  )).length;
  let remainingCapacity = Math.max(0, RAID_SUMMON_MAXIMUM - activeSummonCount);
  if (!remainingCapacity) return [];
  const spawnPlan = ['left', 'right'].map((side) => ({
    side,
    count: randomIntegerBetween(
      definition.patterns.summonStaff.minimumPerSide,
      definition.patterns.summonStaff.maximumPerSide
    )
  }));
  const spawned = [];
  const maximumPlannedPerSide = Math.max(...spawnPlan.map((entry) => entry.count));
  for (let index = 0; index < maximumPlannedPerSide && remainingCapacity > 0; index += 1) {
    for (const { side, count } of spawnPlan) {
      if (index >= count || remainingCapacity <= 0) continue;
...
function resolveBaldKimPattern(runtime, boss, definition, now) {
  ...
  if (cast.pattern === 'wig-rain') {
    ...
    for (const player of players) {
      ...
      if (damageEvent && !damageEvent.dodged && !resistsStatusEffect(player)) {
...
function applyFieldBossMechanics(runtime, now) {
  ...
  for (const boss of runtime.monsters) {
    ...
    if (Number(boss.skillSealedUntil || 0) > now && !boss.currentCast) continue;
    if (boss.raidBoss) {
  ...
  runtime.monsters.forEach((monster) => {
    ...
    for (const stack of monster.poisonStacks) {
      while (stack.nextTickAt <= now && stack.nextTickAt <= stack.expiresAt) {
        const poisonDamage = applyMonsterIncomingDamageIncrease(
          monster,
          Math.max(1, Number(stack.attack) || 1),
          stack.nextTickAt
        );
        monster.hp = Math.max(1, monster.hp - poisonDamage);
        stack.nextTickAt += 1_000;
      }
    }
    if (!Array.isArray(monster.skillDotStacks)) monster.skillDotStacks = [];
    monster.skillDotStacks = monster.skillDotStacks.filter((stack) => stack.expiresAt > now);
    for (const stack of monster.skillDotStacks) {
      const intervalMs = Math.max(100, Number(stack.intervalMs) || 1_000);
      while (stack.nextTickAt <= now && stack.nextTickAt <= stack.expiresAt) {
        const damage = applyMonsterIncomingDamageIncrease(
          monster,
          Math.max(1, Number(stack.damage) || 1),
          stack.nextTickAt
        );
        const minimumHp = stack.nonlethal ? 1 : 0;
        const appliedDamage = Math.min(Math.max(0, monster.hp - minimumHp), damage);
        if (appliedDamage > 0) {
          recordMonsterContribution(monster, stack.userId, appliedDamage);
          monster.hp = Math.max(minimumHp, monster.hp - appliedDamage);
        }
        stack.nextTickAt += intervalMs;
      }
    }
  });
  runtime.monsters.forEach((monster) => advanceMonster(monster, runtime, map, deltaSeconds, now));
  const drainEvents = applyPlayerBuffDrain(runtime, now);
  const protectedDecoys = getActiveDecoys(runtime, now);
  const combatEvents = [
    ...applyFieldBossMechanics(runtime, now),
    ...applyMonsterCounterAttacks(runtime, now, protectedDecoys),
    ...applyContactDamage(runtime, now, protectedDecoys)
  ];
  const combatEventUsers = new Set(combatEvents.map((event) => String(event.userId || '')));
  return [
    ...drainEvents.filter((event) => !combatEventUsers.has(String(event.userId || ''))),
    ...combatEvents
  ];
}

function applyPlayerBuffDrain(runtime, now = Date.now()) {
  const events = [];
  for (const player of runtime.players.values()) {
    const drain = Math.max(0, Math.floor(Number(player.combatProfile?.hpDrain) || 0));
    const intervalMs = Math.max(
      1_000,
      Number(player.combatProfile?.hpDrainIntervalSeconds || 0) * 1000
    );
    if (drain <= 0 || player.currentHp <= 1) continue;
    const lastDrainAt = Number(player.lastBuffDrainAt || player.lastSeenAt || now);
    if (now - lastDrainAt < intervalMs) continue;
    const damage = Math.min(drain, Math.max(0, Number(player.currentHp) - 1));
    player.currentHp = Math.max(1, Number(player.currentHp) - damage);
    player.lastBuffDrainAt = now;
    events.push({
      userId: player.userId,
      source: 'skill-hp-drain',
      damage,
      totalDamage: damage,
      mpDamage: 0,
      blocked: false,
...
function claimWorldControl(userId, clientId, now = Date.now()) {
  ...
  touchOnlineSession(key, now);
  removePlayerFromOtherMaps(key, '');
  return { clientId: sessionId, claimedAt: now };
}

function getWorldControlStatus(userId, clientId) {
  ...
  return { claimed: Boolean(owner), matches };
}

function hasWorldControl(userId, clientId) {
  return getWorldControlStatus(userId, clientId).matches;
}
...
function listRecentWorldControllerIds(now = Date.now()) {
  return Array.from(worldControllers.entries())
    .filter(([, owner]) => {
      const lastSeenAt = Number(owner.lastSeenAt ?? owner.claimedAt) || 0;
      return Math.max(0, Number(now) || 0) - lastSeenAt <= WORLD_CONTROL_ACTIVE_MS;
    })
...
function releaseWorldControl(userId, clientId, { preservePlayer = false } = {}) {
  const key = String(userId);
  if (!hasWorldControl(key, clientId)) return false;
  worldControllers.delete(key);
  if (preservePlayer) {
    for (const runtime of activeMaps.values()) {
      const player = runtime.players.get(key);
      if (player) player.backgroundHunting = true;
    }
  } else {
    removePlayerFromOtherMaps(key, '');
  }
...
function touchOnlineSession(userId, now = Date.now()) {
  const key = String(userId || '');
  if (!key) return 0;
  const touchedAt = Math.max(0, Number(now) || Date.now());
  onlineSessions.set(key, touchedAt);
  return touchedAt;
}

function clearOnlineSession(userId) {
  return onlineSessions.delete(String(userId || ''));
}

function isOnlineSessionActive(userId, now = Date.now()) {
  const touchedAt = Number(onlineSessions.get(String(userId || '')) || 0);
  return touchedAt > 0 && Math.max(0, Number(now) || 0) - touchedAt <= ONLINE_SESSION_TIMEOUT_MS;
}
...
function updatePresence({
  ...
  fireDamageReductionPercent,
  poisonDamageReductionPercent,
  iceDamageReductionPercent,
  lightningDamageReductionPercent,
  holyDamageReductionPercent,
  ...
  magicReflectChance,
  magicReflectPercent,
  magicReflectCapPercent,
  moneyGuardPercent,
  moneyGuardEfficiencyPercent,
  currentMoney,
  hpDrain,
  hpDrainIntervalSeconds,
  stealth,
  periodicHealPercent,
  periodicHealAmount,
  periodicHealIntervalMs,
  ...
  const userKey = String(userId);
  if (!offline) touchOnlineSession(userKey, now);
  const map = getWorldMap(mapId);
  ...
  runtime.players.set(userKey, {
    ...
    equippedTitle: equippedTitle && typeof equippedTitle === 'object'
      ? { ...equippedTitle, stats: { ...(equippedTitle.stats || {}) } }
      : null,
    mapId,
    x: resolvedX,
    floor: resolvedFloor,
    ...
    backgroundHunting: Boolean(offline),
    ...
    lastPotionAt: Number(previous?.lastPotionAt || 0),
    lastBuffDrainAt: Number(previous?.lastBuffDrainAt || now),
    ...
    combatProfile: {
      ...
      physicalDamageReductionPercent: Math.max(
        0,
        Number(
          physicalDamageReductionPercent
          ?? previous?.combatProfile?.physicalDamageReductionPercent
        ) || 0
      ),
      fireDamageReductionPercent: Math.max(0, Number(
        fireDamageReductionPercent ?? previous?.combatProfile?.fireDamageReductionPercent
      ) || 0),
      poisonDamageReductionPercent: Math.max(0, Number(
        poisonDamageReductionPercent ?? previous?.combatProfile?.poisonDamageReductionPercent
      ) || 0),
      iceDamageReductionPercent: Math.max(0, Number(
        iceDamageReductionPercent ?? previous?.combatProfile?.iceDamageReductionPercent
      ) || 0),
      lightningDamageReductionPercent: Math.max(0, Number(
        lightningDamageReductionPercent ?? previous?.combatProfile?.lightningDamageReductionPercent
      ) || 0),
      holyDamageReductionPercent: Math.max(0, Number(
        holyDamageReductionPercent ?? previous?.combatProfile?.holyDamageReductionPercent
      ) || 0),
      ...
      evasion: Math.max(
        0,
        Number(evasion ?? previous?.combatProfile?.evasion) || 0
      ),
      ...
      blockInvincibleSeconds: Math.max(
        0,
        Number(blockInvincibleSeconds ?? previous?.combatProfile?.blockInvincibleSeconds) || 0
      ),
      ...
      damageImmunity: Number(damageImmunity ?? previous?.combatProfile?.damageImmunity) > 0 ? 1 : 0,
      statusImmunity: Number(statusImmunity ?? previous?.combatProfile?.statusImmunity) > 0 ? 1 : 0,
      statusResistance: Math.max(0, Math.min(100, Number(
        statusResistance ?? previous?.combatProfile?.statusResistance
      ) || 0)),
      magicReflectChance: Math.max(0, Math.min(100, Number(
        magicReflectChance ?? previous?.combatProfile?.magicReflectChance
      ) || 0)),
      magicReflectPercent: Math.max(0, Number(
        magicReflectPercent ?? previous?.combatProfile?.magicReflectPercent
      ) || 0),
      magicReflectCapPercent: Math.max(0, Number(
        magicReflectCapPercent ?? previous?.combatProfile?.magicReflectCapPercent
      ) || 0),
      moneyGuardPercent: Math.max(0, Math.min(100, Number(
        moneyGuardPercent ?? previous?.combatProfile?.moneyGuardPercent
      ) || 0)),
      moneyGuardEfficiencyPercent: Math.max(0, Number(
        moneyGuardEfficiencyPercent ?? previous?.combatProfile?.moneyGuardEfficiencyPercent
      ) || 0),
      currentMoney: Math.max(0, Number(currentMoney ?? previous?.combatProfile?.currentMoney) || 0),
      hpDrain: Math.max(0, Number(hpDrain ?? previous?.combatProfile?.hpDrain) || 0),
      hpDrainIntervalSeconds: Math.max(0, Number(
        hpDrainIntervalSeconds ?? previous?.combatProfile?.hpDrainIntervalSeconds
      ) || 0),
...
function attackMonster({
  ...
  const totalDamage = applyRaidBossDamageRule(
    ...
    executed
      ...
      : applyMonsterIncomingDamageIncrease(monster, finalDamage, now),
    damageType,
    now
  );
  ...
  monster.aggroTargetId = userKey;
  if (totalDamage > 0 && monster.hp > 0) armMonsterCounterAttack(monster, userKey, now);
  if (
    !monster.raidBoss
  ...
  if (defeated) {
    ...
    if (monster.fieldBoss && !monster.raidBoss) {
      ...
      drops = queueMonsterDrops(runtime, monster, userKey, now);
    }
  }
  const primaryOutcome = {
    ...
    raidPhaseChanged,
    monsterLevel: monster.level,
    expReward: defeated && !fieldBossReward
      ? Math.floor(Number(monster.expReward) * (
        1 + (
          Number(monster.rewardDebuffUntil || 0) > now
            ? Number(monster.experienceIncreasePercent) || 0
            : 0
        ) / 100
      ))
      : 0,
...
function queueMonsterDrops(runtime, monster, userId, now) {
  ...
  const rewardDropIncrease = Number(monster.rewardDebuffUntil || 0) > now
    ? Math.max(0, Number(monster.dropRateIncreasePercent) || 0)
    : 0;
  const drops = rollMonsterDrops({
    ...monster,
    dropRateMultiplier: 1 + rewardDropIncrease / 100
  }).map((drop, index) => ({
    ...drop,
    id: crypto.randomUUID(),
    userId: ownerId,
...
function applyAugmentChainAttack({
  ...
  const baseChainDamage = Math.max(1, Math.floor(
    Math.max(1, Number(sourceDamage) || 1) * damagePercent / 100
  ));
  const damage = applyMonsterIncomingDamageIncrease(nextMonster, baseChainDamage, now);
...
function useSkillOnMonsters(options = {}) {
  ...
  incomingDamageIncreasePercent = 0,
  monsterAccuracyReductionPercent = 0,
  enemyPhysicalDefenseReductionPercent = 0,
  ...
  poisonMaxStacks = 0,
  dotChance = 0,
  dotDurationSeconds = 0,
  dotDamagePercentOfHit = 0,
  dotIntervalSeconds = 1,
  dotNonlethal = false,
  dotElement = 'neutral',
  temporaryWeaknessElement = '',
  temporaryWeaknessMultiplier = 1,
  temporaryWeaknessSeconds = 0,
  onHitAccuracyDebuffChance = 0,
  onHitAccuracyDebuffPercent = 0,
  onHitMovementSlowChance = 0,
  onHitMovementSlowAmount = 0,
  onHitDebuffSeconds = 0,
  componentDropChance = 0,
  additionalMiscDropChance = 0,
  additionalMiscDropOncePerMonster = false,
  dispelDefenseBuff = false,
  dispelAttackBuff = false,
  dispelEnemyBuffChance = 0,
  lifeStealPercent = 0,
  bonusDamageChance = 0,
  bonusFinalDamagePercent = 0,
  finalHitCriticalChance = 0,
  finalHitCriticalDamagePercent = 200,
  piercingDamageLossPercent = 0,
  closeRangeChance = 0,
  closeRangeDamagePercent = 0,
  executeThresholdPercent = 0,
  executeChance = 0,
  instantKillNormalMonster = false,
  undeadOnly = false,
  ...
  if (progressivePiercing) {
    ...
    candidates = inRange
      ...
      .slice(0, targetLimit);
  } else if (chainFromPrimaryTarget) {
    const primaryTarget = requestedMonster || [...inRange].sort((left, right) => (
      Math.abs(Number(left.x) - Number(player.x))
      - Math.abs(Number(right.x) - Number(player.x))
    ))[0];
    const chainRangePercent = Math.max(1, Math.min(300, Number(rangePx) || 300))
      / worldWidth * 100;
    candidates = primaryTarget
      ? [
        ...
        ...inRange
          .filter((monster) => (
            monster.id !== primaryTarget.id
            && Math.abs(Number(monster.x) - Number(primaryTarget.x)) <= chainRangePercent + 4.5
          ))
          .sort((left, right) => (
            Math.abs(Number(left.x) - Number(primaryTarget.x))
            - Math.abs(Number(right.x) - Number(primaryTarget.x))
          ))
      ].slice(0, targetLimit)
      : [];
  } else {
  ...
  for (const [targetIndex, monster] of candidates.entries()) {
    const piercingDamagePercent = progressivePiercing
      ...
      : Math.max(
        ...
        (Number(skillPercent) || 100) * (
          piercing ? 1 - Math.max(0, Number(piercingDamageLossPercent) || 0) * targetIndex / 100 : 1
        )
      );
    ...
    const wasBelowExecuteThreshold = monster.hp / Math.max(1, monster.maxHp) * 100
      <= Number(executeThresholdPercent || 0);
    const normalMonsterExecuted = dealDamage
      && !leaveAtOneHp
      && instantKillNormalMonster
      && !monster.fieldBoss
      && !monster.raidBoss;
    const executed = normalMonsterExecuted || (
      dealDamage
      && !leaveAtOneHp
      && closeRangeTriggered
      && wasBelowExecuteThreshold
    ...
    for (let hit = 0; !executed && dealDamage && hit < hitCount && monster.hp > 0; hit += 1) {
      const defense = getEffectiveMonsterDefense(monster, damageType, now);
      const multiplier = getSkillElementMultiplier(monster, activeElements, now);
      const isFinalHit = hit === hitCount - 1;
      const resolvedCriticalChance = isFinalHit && Number(finalHitCriticalChance) > 0
        ? Number(finalHitCriticalChance)
        : Number(criticalChance);
      const critical = (rollCriticalPerHit || (isFinalHit && Number(finalHitCriticalChance) > 0))
        && Math.random() * 100 < Math.max(0, resolvedCriticalChance || 0);
      const criticalMultiplier = critical
        ? Math.max(
          1,
          Number(
            isFinalHit && Number(finalHitCriticalChance) > 0
              ? finalHitCriticalDamagePercent
              : criticalDamagePercent
          ) || 200
        ) / 100
        : 1;
      const bonusMultiplier = Number(bonusDamageChance) > 0
        && Math.random() * 100 < Number(bonusDamageChance)
        ? 1 + Math.max(0, Number(bonusFinalDamagePercent) || 0) / 100
        : 1;
      const resolvedDamage = resolveOutgoingDamage({
        damage: Number(baseDamage) * bonusMultiplier
          * (damageRange ? closeRangeMultiplier : criticalMultiplier * closeRangeMultiplier),
        damageRange: targetDamageRange
          ? scaleDamageRange(
            targetDamageRange,
            criticalMultiplier * closeRangeMultiplier * bonusMultiplier
          )
          : null,
        damageType,
      ...
      const damageBeforeRaidRule = splitDamageAcrossHits && hitCount > 1
        ...
        : resolvedDamage;
      const damage = applyRaidBossDamageRule(
        monster,
        applyMonsterIncomingDamageIncrease(monster, damageBeforeRaidRule, now),
        damageType,
        now
      );
    ...
    for (let attempt = 0; attempt < 1 + repeatedEffectPasses; attempt += 1) {
      const hasCombatDebuff = Number(outgoingDamageReductionPercent) > 0
        || Number(incomingDamageIncreasePercent) > 0
        || Number(monsterAccuracyReductionPercent) > 0
        || Number(enemyPhysicalDefenseReductionPercent) > 0
        || Number(enemyDefenseIncreasePercent) > 0
        || Number(movementSpeedReduction) > 0
        || Number(bindChance) > 0
        || Number(skillSealChance) > 0
        || Number(experienceIncreasePercent) > 0
        || Number(dropRateIncreasePercent) > 0;
      if (
        hasCombatDebuff
        && !monster.raidBoss
        && Math.random() * 100 < Number(debuffChance || 0)
      ) {
        const debuffUntil = now + Math.max(0, Number(debuffDurationSeconds) || 0) * 1000;
        ...
        monster.incomingDamageIncreasePercent = Math.max(
          Number(monster.incomingDamageIncreasePercent) || 0,
          Number(incomingDamageIncreasePercent) || 0
        );
        monster.incomingDamageDebuffUntil = Math.max(
          Number(monster.incomingDamageDebuffUntil) || 0,
          debuffUntil
        );
        monster.monsterAccuracyReductionPercent = Math.max(
          Number(monster.monsterAccuracyReductionPercent) || 0,
          Number(monsterAccuracyReductionPercent) || 0
        );
        monster.monsterAccuracyDebuffUntil = Math.max(
          Number(monster.monsterAccuracyDebuffUntil) || 0,
          debuffUntil
        );
        monster.physicalDefenseReductionPercent = Math.max(
          Number(monster.physicalDefenseReductionPercent) || 0,
          Number(enemyPhysicalDefenseReductionPercent) || 0
        );
        monster.physicalDefenseReductionUntil = Math.max(
          Number(monster.physicalDefenseReductionUntil) || 0,
          debuffUntil
        );
        monster.defenseIncreasePercent = Math.max(
          Number(monster.defenseIncreasePercent) || 0,
          Number(enemyDefenseIncreasePercent) || 0
        );
        monster.defenseModifierUntil = Math.max(
          Number(monster.defenseModifierUntil) || 0,
          debuffUntil
        );
        monster.movementSpeedReduction = Math.max(
          Number(monster.movementSpeedReduction) || 0,
          Number(movementSpeedReduction) || 0
        );
        monster.movementSpeedDebuffUntil = Math.max(
          Number(monster.movementSpeedDebuffUntil) || 0,
          debuffUntil
        );
        if (Number(bindChance) > 0 && Math.random() * 100 < Number(bindChance)) {
          monster.stunnedUntil = Math.max(
            Number(monster.stunnedUntil) || 0,
            now + Math.max(0, Number(bindSeconds) || 0) * 1000
          );
        }
        if (Number(skillSealChance) > 0 && Math.random() * 100 < Number(skillSealChance)) {
          monster.skillSealedUntil = Math.max(
            Number(monster.skillSealedUntil) || 0,
            now + Math.max(0, Number(skillSealSeconds) || 0) * 1000
          );
        }
        monster.experienceIncreasePercent = Math.max(
          Number(monster.experienceIncreasePercent) || 0,
          Number(experienceIncreasePercent) || 0
        );
        monster.dropRateIncreasePercent = Math.max(
          Number(monster.dropRateIncreasePercent) || 0,
          Number(dropRateIncreasePercent) || 0
        );
        monster.rewardDebuffUntil = Math.max(
          Number(monster.rewardDebuffUntil) || 0,
          debuffUntil
        );
        if (debuffSkillId) {
          monster.combatDebuffSkillId = String(debuffSkillId);
          monster.combatDebuffName = String(debuffName || debuffSkillId);
          monster.combatDebuffUntil = Math.max(
            Number(monster.combatDebuffUntil) || 0,
            debuffUntil
          );
        }
        debuffApplied = true;
    ...
    const onHitUntil = now + Math.max(0, Number(onHitDebuffSeconds) || 0) * 1000;
    if (
      totalDamage > 0
      && Number(onHitAccuracyDebuffChance) > 0
      && Math.random() * 100 < Number(onHitAccuracyDebuffChance)
    ) {
      monster.monsterAccuracyReductionPercent = Math.max(
        Number(monster.monsterAccuracyReductionPercent) || 0,
        Number(onHitAccuracyDebuffPercent) || 0
      );
      monster.monsterAccuracyDebuffUntil = Math.max(
        Number(monster.monsterAccuracyDebuffUntil) || 0,
        onHitUntil
    ...
    if (
      totalDamage > 0
      && Number(onHitMovementSlowChance) > 0
      && Math.random() * 100 < Number(onHitMovementSlowChance)
    ) {
      monster.movementSpeedReduction = Math.max(
        Number(monster.movementSpeedReduction) || 0,
        Number(onHitMovementSlowAmount) || 0
      );
      monster.movementSpeedDebuffUntil = Math.max(
        Number(monster.movementSpeedDebuffUntil) || 0,
        onHitUntil
    ...
    if (
      totalDamage > 0
      && monster.hp > 0
      && Number(dotDurationSeconds) > 0
      && Number(dotDamagePercentOfHit) > 0
      && Math.random() * 100 < Math.max(0, Number(dotChance) || 100)
    ) {
      if (!Array.isArray(monster.skillDotStacks)) monster.skillDotStacks = [];
      const durationMs = Math.max(100, Number(dotDurationSeconds) * 1000);
      monster.skillDotStacks.push({
        userId: userKey,
        element: String(dotElement || element || 'neutral'),
        damage: Math.max(1, Math.floor(totalDamage * Number(dotDamagePercentOfHit) / 100)),
        intervalMs: Math.max(100, Number(dotIntervalSeconds) * 1000),
        nextTickAt: now + Math.max(100, Number(dotIntervalSeconds) * 1000),
        expiresAt: now + durationMs,
        nonlethal: Boolean(dotNonlethal)
      });
      monster.skillDotStacks = monster.skillDotStacks.slice(-8);
      dotApplied = true;
    }
    if (totalDamage > 0 && temporaryWeaknessElement && Number(temporaryWeaknessSeconds) > 0) {
      monster.temporaryWeaknessElement = String(temporaryWeaknessElement);
      monster.temporaryWeaknessMultiplier = Math.max(1, Number(temporaryWeaknessMultiplier) || 1);
      monster.temporaryWeaknessUntil = Math.max(
        Number(monster.temporaryWeaknessUntil) || 0,
        now + Number(temporaryWeaknessSeconds) * 1000
      );
    }
    const dispelChance = Math.max(
      Number(dispelEnemyBuffChance) || 0,
      (dispelDefenseBuff || dispelAttackBuff) ? Number(debuffChance) || 0 : 0
    );
    const dispelled = dispelChance > 0 && Math.random() * 100 < dispelChance;
    if (dispelled) {
      if (dispelDefenseBuff || Number(dispelEnemyBuffChance) > 0) {
        monster.physicalImmuneUntil = 0;
        monster.defenseIncreasePercent = 0;
        monster.defenseModifierUntil = 0;
      }
      if (dispelAttackBuff || Number(dispelEnemyBuffChance) > 0) {
        monster.attackBuffPercent = 0;
        monster.attackBuffUntil = 0;
      }
    }
    ...
    const freezeMultiplier = getSkillElementMultiplier(monster, activeElements, now);
    ...
    if (
      pull
      && !monster.raidBoss
      && Math.random() * 100 < Math.max(0, Number(pullChance) || 0)
    ) {
      monster.x = clamp(player.x + (player.facingLeft ? -2 : 2), 2, 92);
      knockedBack = true;
    } else if (
      !monster.raidBoss
      && Number(knockbackChance) > 0
      && Math.random() * 100 < Number(knockbackChance)
    ) {
      const direction = Number(monster.x) >= Number(player.x) ? 1 : -1;
      const bounds = getPlatformBounds(getMonsterPlatform(mapId, monster));
      monster.x = clamp(
        Number(monster.x) + direction * Number(knockbackDistancePx) / worldWidth * 100,
        bounds.minimum,
        bounds.maximum
      );
      knockedBack = true;
    } else {
      knockedBack = applyHeavyHitKnockback(monster, player, totalDamage, mapId);
    }
    let defeated = monster.hp <= 0;
    let raidPhaseChanged = false;
    if (defeated && monster.raidBoss) {
    ...
    if (defeated) {
      ...
      armMonsterCounterAttack(monster, userKey, now);
    }
    const componentDropCount = totalDamage > 0
      && Number(componentDropChance) > 0
      && Math.random() * 100 < Number(componentDropChance)
      ? 1
      : 0;
    let additionalMiscDropGranted = false;
    const miscDropAlreadyGranted = Boolean(monster.additionalMiscGrantedByUser?.[userKey]);
    if (
      totalDamage > 0
      && monster.lootItemId
      && Number(additionalMiscDropChance) > 0
      && (!additionalMiscDropOncePerMonster || !miscDropAlreadyGranted)
      ...
      additionalMiscDropGranted = true;
    }
    outcomes.push({
      monsterId: monster.id,
      speciesId: monster.speciesId,
      ...
      defeated,
      monsterLevel: monster.level,
      expReward: defeated && !monster.fieldBoss
        ? Math.floor(Number(monster.expReward) * (
          1 + (
            Number(monster.rewardDebuffUntil || 0) > now
              ? Number(monster.experienceIncreasePercent) || 0
      ...
      debuffApplied,
      dispelled,
      componentsDropped: componentDropCount,
      additionalMiscDropped: additionalMiscDropGranted,
      debuffApplications,
      stunApplications,
      hitResults,
      targetX,
  ...
  return {
    ...
    lifeStolen: Math.floor(
      outcomes.reduce((sum, outcome) => sum + Number(outcome.damage || 0), 0)
        * Math.max(0, Number(lifeStealPercent) || 0) / 100
    ),
    componentsDropped: outcomes.reduce(
      (sum, outcome) => sum + Number(outcome.componentsDropped || 0),
      0
    ),
    fieldBossRewards,
    casterMovement,
...
function updatePlayerResources(userId, resources = {}, options = {}) {
  const potionUsedAt = options.reason === 'potion'
    ? Math.max(0, Number(options.now) || Date.now())
    : 0;
  for (const runtime of activeMaps.values()) {
    ...
    if (potionUsedAt) {
      player.lastPotionAt = Math.max(Number(player.lastPotionAt) || 0, potionUsedAt);
    }
  }
}

function getPlayerResourceState(userId, mapId = '') {
  const userKey = String(userId || '');
  const preferredRuntime = activeMaps.get(String(mapId || ''));
  const player = preferredRuntime?.players.get(userKey)
    || Array.from(activeMaps.values()).map((runtime) => runtime.players.get(userKey)).find(Boolean);
  if (!player) return null;
  return {
    currentHp: Math.max(0, Number(player.currentHp) || 0),
    maxHp: Math.max(1, Number(player.maxHp) || 1),
    currentMp: Math.max(0, Number(player.currentMp) || 0),
    maxMp: Math.max(0, Number(player.maxMp) || 0),
    lastPotionAt: Math.max(0, Number(player.lastPotionAt) || 0),
    backgroundHunting: Boolean(player.backgroundHunting)
  };
}
