...
function applyMonsterCounterAttacks(runtime, now) {
  const events = [];
  ...
  for (const monster of runtime.monsters) {
    ...
    const rangePercent = Math.max(1, Number(monster.counterAttackRangePx) || 180)
      / worldWidth * 100;
    if (
      Number(player.floor) !== Number(monster.floor)
      || Math.abs(Number(player.x) - Number(monster.x)) > rangePercent + 4.5
    ) {
      monster.state = 'chase';
      continue;
    }

    monster.direction = Number(player.x) < Number(monster.x) ? -1 : 1;
    ...
    const calculation = calculateIncomingPhysicalDamage({
      monsterAttack: rollMonsterCounterAttackDamage(monster) * (1 - outgoingReduction / 100),
    ...
    const passiveDodgeChance = Math.max(
      0,
      Math.min(100, Number(player.combatProfile.dodgeChance) || 0)
    );
    ...
    const effectiveDodgeChance = 100
      - (100 - passiveDodgeChance) * (1 - accuracyReductionPercent / 100);
...
function tickRuntime(runtime, now) {
  ...
  const combatEvents = [
    ...
    ...applyMonsterCounterAttacks(runtime, now),
