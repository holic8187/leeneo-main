...
function renderMonsters(monsters = []) {
  ...
  monsters.forEach((monster) => {
    ...
    const warCryActive = monster.combatDebuffSkillId === 'war_cry'
      && Number(monster.combatDebuffUntil) > Number(state.worldServerTime || Date.now());
    if (debuffMarker) {
      debuffMarker.textContent = warCryActive ? '📣 공격↓ 피해↑ 명중↓' : '';
      debuffMarker.classList.toggle('hidden', !warCryActive);
    }
    element.classList.toggle(
      'is-frozen',
