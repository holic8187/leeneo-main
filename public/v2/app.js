...
function renderPartyCombatHud() {
  ...
  hud.innerHTML = members.map((member) => {
    ...
    return `<div class="party-combat-member${member.isSelf ? ' is-self' : ''}${member.online ? '' : ' is-offline'}${sameMap ? '' : ' is-away'}">
...
function renderMonsters(monsters = []) {
  ...
  monsters.forEach((monster) => {
    ...
    const raidBuff = element.querySelector('.raid-boss-buff');
...
function renderWorldEntities(data = {}) {
  ...
  if (ownContact) {
    ...
    if (Number(ownContact.currentHp) > 0) {
      ...
      setWorldActivity(
        ownContact.dodged
          ? `${resourceCrash ? '자원 감축' : '패시브'} 회피 성공 · 2초 무적`
          : resourceCrash
...
async function performQuickPotion(slot, automatic = false) {
  ...
  if (automatic && (
    ...
    || state.potionUseBusy
    || state.manualPotionRequestsInFlight > 0
  )) return;
  if (automatic) {
