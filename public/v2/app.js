...
function renderWorldMap(mapId, arrivalPortalIndex = 0, explicitArrivalX = null) {
...
function shouldUseBackgroundHunting() {
  return Boolean(
    ...
    && (document.visibilityState !== 'visible' || !state.worldStageVisible)
  );
}
...
async function leaveWorldControlForBackgroundHunting() {
  ...
  state.moving = false;
  try {
    await request('/api/v2/world/leave', {
      method: 'POST',
      body: JSON.stringify({
        clientId: state.worldClientId,
        mapId: state.currentMapId,
        ...
        <div class="shop-item-list">
          ${sellable.length ? sellable.map((item) => `<article>
            ...
            <div class="shop-item-actions ${item.ammunitionType === 'throwing-star' ? 'is-throwing-star' : ''}">
              ${item.itemType === 'ammunition' && item.ammunitionType === 'throwing-star'
...
function eventBody() {
  ...
  return `<div class="event-sheet">
    <header>
      <span>2026.07.06 - ${escapeHtml(eventEndLabel)}</span>
      <h3>기간제 정착 지원 이벤트</h3>
...
document.addEventListener('visibilitychange', updateBackgroundHuntingHandoff);
if (typeof IntersectionObserver === 'function' && $('worldStage')) {
  const worldStageObserver = new IntersectionObserver((entries) => {
    state.worldStageVisible = Boolean(entries[0]?.isIntersecting);
    updateBackgroundHuntingHandoff();
  }, { threshold: 0.01 });
  worldStageObserver.observe($('worldStage'));
}
