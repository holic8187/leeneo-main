...
const state = {
  ...
  equipmentTab: 'weapon',
  titleAwardQueue: [],
  queuedTitleAwardIds: new Set(),
  activeTitleAward: null,
...
function titleEffectClass(title) {
  const effect = String(title?.effectClass || '').replace(/[^a-z0-9-]/gi, '');
  return effect ? `title-effect-${effect}` : '';
}

function applyEquippedTitleElement(element, title) {
  if (!element) return;
  const previousEffect = Array.from(element.classList)
    .filter((className) => className.startsWith('title-effect-'));
  element.classList.remove('title-grade-common', 'title-grade-rare', 'title-grade-heroic', ...previousEffect);
  element.replaceChildren();
  element.classList.toggle('hidden', !title?.name);
  if (!title?.name) {
  ...
  element.setAttribute('aria-label', `${title.name} 칭호`);
  element.classList.add(`title-grade-${title.grade || 'common'}`);
  const effectClass = titleEffectClass(title);
  if (effectClass) element.classList.add(effectClass);
}

function enqueueTitleAwards(titles = {}) {
  for (const title of titles.pendingAwards || []) {
    const id = String(title?.id || '');
    if (!id || state.queuedTitleAwardIds.has(id)) continue;
    state.queuedTitleAwardIds.add(id);
    state.titleAwardQueue.push(title);
  }
  showNextTitleAward();
}

function showNextTitleAward() {
  if (state.activeTitleAward || !state.titleAwardQueue.length) return;
  const title = state.titleAwardQueue.shift();
  const modal = $('titleAwardModal');
  const sheet = $('titleAwardSheet');
  if (!modal || !sheet) return;
  state.activeTitleAward = title;
  $('titleAwardIcon').textContent = title.icon || '◆';
  $('titleAwardName').textContent = title.name || '새 칭호';
  $('titleAwardBannerName').textContent = title.name || '새 칭호';
  $('titleAwardCondition').textContent = `획득 조건 · ${title.condition || '비공개 조건 달성'}`;
  $('titleAwardEffect').textContent = `${title.gradeName || title.grade || '칭호'} · ${title.effectDescription || ''}`;
  sheet.className = `title-award-sheet title-grade-${title.grade || 'common'} ${titleEffectClass(title)}`.trim();
  modal.classList.remove('hidden');
  document.body.classList.add('modal-open');
}

async function confirmTitleAward() {
  const title = state.activeTitleAward;
  const button = $('titleAwardConfirm');
  if (!title || !button || button.disabled) return;
  button.disabled = true;
  try {
    await request('/api/v2/titles/acknowledge', {
      method: 'POST',
      body: JSON.stringify({ titleId: title.id })
    });
    if (state.character?.titles?.pendingAwards) {
      state.character.titles.pendingAwards = state.character.titles.pendingAwards
        .filter((entry) => entry.id !== title.id);
    }
  } catch (err) {
    setWorldActivity(err.message);
  } finally {
    $('titleAwardModal')?.classList.add('hidden');
    document.body.classList.remove('modal-open');
    state.activeTitleAward = null;
...
function renderGame(data) {
  ...
  const ownCharacterTag = $('fieldCharacter')?.querySelector('.character-tag');
  if (ownCharacterTag) ownCharacterTag.textContent = state.displayName || '사원';
  applyEquippedTitleElement(
    $('fieldCharacter')?.querySelector('.character-title'),
    character.titles?.equipped
  );
  enqueueTitleAwards(character.titles);
...
function ensureRemotePlayerElement(player) {
  ...
  element.innerHTML = `
    ...
    <span class="remote-player-tag"><b></b><em class="remote-player-title hidden"></em><small></small></span>
  ...
  element._v2Refs = {
    ...
    title: element.querySelector('.remote-player-title'),
...
function renderRemotePlayers(players = []) {
  ...
  players.filter((player) => player.userId !== state.selfUserId).forEach((player) => {
    ...
    applyEquippedTitleElement(refs.title, player.equippedTitle);
...
function renderWorldEntities(data = {}) {
  ...
  if (data.titles && state.character) {
    state.character.titles = data.titles;
    applyEquippedTitleElement(
      $('fieldCharacter')?.querySelector('.character-title'),
      data.titles.equipped
    );
    enqueueTitleAwards(data.titles);
  }
...
function titleInventoryPanel() {
  if (state.inventoryTab !== 'equipment') return '';
  const titles = state.character?.titles || { owned: [], equipped: null };
  const equippedId = String(titles.equipped?.id || '');
  return `<section class="title-inventory-panel">
    <header>
      <div><span>◆</span><strong>보유 칭호</strong></div>
      <small>획득하기 전까지 칭호와 조건은 공개되지 않습니다.</small>
    </header>
    <div class="title-inventory-list">
      ${(titles.owned || []).length ? titles.owned.map((title) => `<article
        class="title-inventory-card title-grade-${escapeHtml(title.grade || 'common')} ${escapeHtml(titleEffectClass(title))}">
        <span>${escapeHtml(title.icon || '◆')}</span>
        <div>
          <strong class="title-card-banner"><span>${escapeHtml(title.name)}</span></strong>
          <small>${escapeHtml(equipmentStatText({ stats: title.stats }))}</small>
          <em>획득 조건 · ${escapeHtml(title.condition)}</em>
        </div>
        <button type="button" data-equip-title="${escapeHtml(title.id)}"
          ${equippedId === title.id ? 'disabled' : ''}>${equippedId === title.id ? '장착 중' : '장착'}</button>
      </article>`).join('') : '<p class="notice-line">아직 획득한 칭호가 없습니다.</p>'}
    </div>
  </section>`;
}
...
function bindInventoryControls() {
  ...
  document.querySelectorAll('[data-equip-title]').forEach((button) => {
    button.addEventListener('click', () => equipOwnedTitle(button.dataset.equipTitle));
  });
...
function rerenderTitleSurfaces() {
  if (state.activeFeature === 'inventory') rerenderInventory();
  if (state.activeFeature === 'equipment') {
    $('featureBody').innerHTML = equipmentBody();
    bindEquipmentTabs();
  }
}
...
async function equipOwnedTitle(titleId) {
  try {
    const data = await request('/api/v2/titles/equip', {
      method: 'POST',
      body: JSON.stringify({ titleId })
    });
    state.character = data.character;
    renderGame({ preview: state.preview, character: data.character, displayName: state.displayName });
    rerenderTitleSurfaces();
    setWorldActivity(`${data.title.name} 칭호를 장착했습니다.`);
  } catch (err) {
    setWorldActivity(err.message);
  }
}

async function unequipOwnedTitle() {
  try {
    const data = await request('/api/v2/titles/unequip', {
      method: 'POST',
      body: JSON.stringify({})
    });
    state.character = data.character;
    renderGame({ preview: state.preview, character: data.character, displayName: state.displayName });
    rerenderTitleSurfaces();
    setWorldActivity(`${data.title.name} 칭호를 해제했습니다.`);
  } catch (err) {
    setWorldActivity(err.message);
  }
}
...
function equipmentBody() {
  ...
  const slots = activeTab.slots.map((slot) => {
    ...
    return `<article class="equipment-slot ${item ? 'is-equipped' : 'is-empty'}">
      ...
      ${item ? (slot.key === 'title'
        ? '<button type="button" data-unequip-title>해제</button>'
        : `<button type="button" data-unequip-equipment="${slot.key}">해제</button>`
      ) : ''}
    </article>`;
  }).join('');
  return `<div class="equipment-sheet">
    ...
    <div class="equipment-actions">
      <button type="button" data-open-enhancement ${Object.entries(loadout).some(([slot, item]) => slot !== 'title' && Boolean(item)) ? '' : 'disabled'}>장비 강화</button>
    </div>
...
function enhancementBody() {
  const loadout = state.character?.equipmentLoadout || {};
  const equipped = Object.entries(loadout).filter(([slot, item]) => slot !== 'title' && item?.itemId);
  if (!equipped.length) {
...
function bindEquipmentTabs() {
  ...
  document.querySelector('[data-unequip-title]')?.addEventListener('click', unequipOwnedTitle);
...
function questRewardText(rewards = {}) {
  ...
  for (const pool of rewards.randomRewards || []) {
    parts.push(pool.name || '무작위 보상 1종');
  }
...
function specialActionsBody() {
  ...
  return `<div class="special-action-sheet">
    ...
    <div class="special-action-list">
      ...
      <article>
        <div class="special-action-icon">🧰</div>
        <div><strong>부업</strong><p>몬스터 전리품과 보스 재료로 152레벨 무기를 제작합니다.</p><small>행동력 4 · 기본 성공률 90%</small></div>
...
function sideJobCraftingBody() {
  ...
  return `<div class="side-job-sheet">
    ...
    <div class="crafting-recipe-list">${crafting.recipes.map((recipe) => {
      ...
      const weaponTypeLabel = ITEM_WEAPON_TYPE_LABELS[weapon.weaponType] || weapon.weaponType || '무기';
      return `<article>
        <header><span>${escapeHtml(weapon.icon || '⚔️')}</span><div>
          <strong class="crafting-recipe-name" title="${escapeHtml(weapon.name || '제작 무기')}">${escapeHtml(weapon.name || '제작 무기')}</strong>
          <small class="crafting-recipe-type">무기 종류 · ${escapeHtml(weaponTypeLabel)}</small>
          <small class="crafting-recipe-stats">Lv.${formatNumber(weapon.requiredLevel || 152)} · ${escapeHtml(craftingStatText(weapon.stats))}</small></div>
...
$('titleAwardConfirm')?.addEventListener('click', confirmTitleAward);
