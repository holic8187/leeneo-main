...
function renderSkillQuickbar() {
  ...
  quickbar.querySelectorAll('[data-use-skill]').forEach((button) => {
    bindImmediateSkillButton(button, () => queueManualSkillUse(button.dataset.useSkill));
  });
...
function bindImmediateSkillButton(button, handler) {
  if (!button) return;
...
async function performSkillPresetUpdate(skillId, preferredSlot = null) {
  ...
  if (Number.isInteger(preferredSlot)) {
    try {
      const data = await request('/api/v2/skills/preset', {
        method: 'POST',
        body: JSON.stringify({ skillId, slotIndex: preferredSlot })
      });
...
function updateSkillPreset(skillId, preferredSlot = null) {
  const queued = state.skillPresetUpdateQueue
    .catch(() => {})
    .then(() => performSkillPresetUpdate(skillId, preferredSlot));
  state.skillPresetUpdateQueue = queued;
  return queued;
}
...
function applyChannelSkillHit(hit = {}, impactPoint = null) {
  const element = Array.from($('monsterLayer').children).find(
    (node) => node.dataset.monsterId === hit.monsterId
  );
  const amount = hit.missed ? 'MISS' : (hit.displayDamage ?? hit.damage);
  const kind = !hit.missed && hit.critical ? 'critical' : 'outgoing';
  const sequence = hit.projectileIndex ?? hit.hitIndex;
  if (element?.isConnected) {
    showFloatingDamage(element, amount, kind, sequence);
  } else {
    const fallbackPoint = impactPoint
      || (typeof getWorldStagePoint === 'function'
        ? getWorldStagePoint(hit.targetX, hit.targetFloor)
        : null);
    if (fallbackPoint && typeof showFloatingDamageAtPoint === 'function') {
      showFloatingDamageAtPoint(fallbackPoint, amount, kind, sequence);
    }
  }
  const remainingHp = Number(hit.visualRemainingHp ?? hit.remainingHp);
  if (remainingHp <= 0) {
    state.worldMonsters = state.worldMonsters.filter(
  ...
  state.worldMonsters = state.worldMonsters.map((monster) => (
    monster.id === hit.monsterId
      ? { ...monster, hp: remainingHp, maxHp: Number(hit.maxHp) || monster.maxHp }
      : monster
  ));
  const hpBar = element?.querySelector('.monster-hp i');
  if (hpBar) hpBar.style.width = `${ratio(remainingHp, hit.maxHp)}%`;
}
...
function prepareProgressivePiercingTarget(skill = {}) {
  ...
  return { target, targets: piercingTargets, direction };
}

function queueManualSkillUse(skillId) {
  if (!skillId || state.dead) return false;
  state.manualSkillQueue.push(String(skillId));
  beginManualSkillPriority();
  void drainManualSkillQueue();
  return true;
}

async function drainManualSkillQueue() {
  if (state.manualSkillQueueRunning) return;
  state.manualSkillQueueRunning = true;
  try {
    while (state.manualSkillQueue.length && !state.dead) {
      while (state.skillUseBusy && !state.dead) await new Promise((resolve) => setTimeout(resolve, 8));
...
async function useActiveSkill(skillId, options = {}) {
  ...
  if (
    ...
    const pendingCombatTargetIds = new Set();
    let preparedPiercingTarget = null;
    const retainTargetIds = (targetIds = []) => {
      const ids = targetIds.map(String).filter(Boolean);
      ids.forEach((id) => pendingCombatTargetIds.add(id));
      if (typeof retainCombatVisualTargets === 'function') retainCombatVisualTargets(ids);
    };
    const retainPreparedTargets = (prepared) => {
      retainTargetIds((prepared?.targets || []).map((monster) => monster.id));
    };
    state.skillUseBusy = true;
    try {
      const offensive = ['enemy', 'enemies'].includes(skill.target);
      if (offensive) {
        preparedPiercingTarget = prepareProgressivePiercingTarget(skill);
        retainPreparedTargets(preparedPiercingTarget);
      }
    ...
    const requestSkillUse = () => {
      if (progressivePiercingSkill) {
        preparedPiercingTarget = prepareProgressivePiercingTarget(skill);
        if (!preparedPiercingTarget) {
          return Promise.reject(new Error('사거리 안에 관통할 대상이 없습니다.'));
        }
        retainPreparedTargets(preparedPiercingTarget);
      }
      const travelDirection = progressivePiercingSkill
        ? Number(preparedPiercingTarget?.direction) || 1
        : (typeof window.getSkillTravelDirection === 'function'
          ? window.getSkillTravelDirection()
          : ($('fieldCharacter')?.classList.contains('facing-left') ? -1 : 1));
      const facingLeft = travelDirection < 0;
      return request('/api/v2/skills/use', {
        ...
        body: JSON.stringify({
          ...
          targetId: progressivePiercingSkill
            ? String(preparedPiercingTarget?.target?.id || '')
    ...
    if (channelDurationSeconds > 0 && typeof playChanneledSkillMotion === 'function') {
      const startingMonsters = new Map((state.worldMonsters || []).map((monster) => [
        String(monster.id),
        { ...monster }
      ]));
      retainTargetIds([state.combatTargetId]);
      data = await requestSkillUse();
      if (options.automatic && data.character) {
        state.character = data.character;
        preloadNextAutoSkillForCombat();
      }
      const channelHits = data.combat?.channel?.hitResults || [];
      const channelTargetIds = [...new Set(
        channelHits.map((hit) => String(hit.monsterId || '')).filter(Boolean)
      )];
      retainTargetIds(channelTargetIds);
      const restoredMonsters = [...(state.worldMonsters || [])];
      channelTargetIds.forEach((monsterId) => {
        const initial = startingMonsters.get(monsterId);
        if (!initial) return;
        const existingIndex = restoredMonsters.findIndex(
          (monster) => String(monster.id) === monsterId
        );
        if (existingIndex >= 0) restoredMonsters[existingIndex] = { ...initial };
        else restoredMonsters.push({ ...initial });
      });
      if (typeof renderMonsters === 'function') renderMonsters(restoredMonsters);
      channelTargetIds.forEach((monsterId) => {
        const initial = startingMonsters.get(monsterId);
        if (!initial) return;
        const element = Array.from($('monsterLayer')?.children || []).find(
          (node) => node.dataset.monsterId === monsterId
        );
        const hpBar = element?.querySelector('.monster-hp i');
        if (hpBar) hpBar.style.width = `${ratio(initial.hp, initial.maxHp)}%`;
      });
      ...
      const tripleOutcome = data.combat?.outcomes?.[0] || {};
      const rawHitResults = (tripleOutcome.hitResults || [])
        .map((hit) => ({
          ...hit,
          critical: Boolean(hit.critical),
          targetX: tripleOutcome.targetX,
          targetFloor: tripleOutcome.targetFloor
        }));
    ...
    if (data.combat) {
      ...
      if (data.combat.teleport) {
        ...
        if (character) {
          const teleportXValue = Number(teleport.x);
          const teleportX = Number.isFinite(teleportXValue)
            ? Math.max(0, Math.min(94, teleportXValue))
            : 8;
          character.classList.add('is-teleporting');
          character.classList.toggle('facing-left', teleport.direction === 'left');
          character.style.transitionDuration = '0ms';
          character.style.left = `${teleportX}%`;
          setCharacterFloor(
            Math.max(0, Math.floor(Number(teleport.floor) || 0)),
            teleportX
          );
      ...
      if (progressivePiercing) {
        const outcomeIds = (data.combat.outcomes || [])
          ...
          .filter(Boolean);
        outcomeIds.forEach((id) => pendingCombatTargetIds.add(id));
    ...
    if (preservePotionState && data.character && state.character) {
      data.character.resources = { ...(state.character.resources || {}) };
      data.character.inventory = state.character.inventory || data.character.inventory;
    }
    const inventory = preservePotionState
      ? null
      : (data.inventory || data.character?.inventory);
    ...
    try {
      if (typeof releaseCombatVisualTargets === 'function') {
        releaseCombatVisualTargets([...pendingCombatTargetIds]);
      }
    } catch (cleanupError) {
      console.error('V2 skill visual cleanup error:', cleanupError);
    } finally {
