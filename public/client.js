...
const PATCH_NOTES = [
  {
    id: '2026-06-30-v2-render-cutover-switch',
    time: '2026-06-30 20:00',
    title: '기존 Render 주소 V2 전환 스위치',
    details: [
      'Render 환경변수 APP_MODE를 v2로 설정하면 기존 서비스 주소가 자동으로 /v2/ 화면을 열도록 준비했습니다.',
      'V2 모드에서는 기존 V1 API와 V1 주간 면담 정산 작업을 중지해 라이브 데이터가 뒤에서 변경되지 않게 했습니다.',
      '기본 APP_MODE는 v1이므로 실제 전환 전까지 현재 게임은 그대로 유지됩니다.'
    ]
  },
  {
    id: '2026-06-30-hoi-company-brand-rename',
    time: '2026-06-30 19:20',
    title: '호이상사 브랜드명 변경',
    details: [
      '게임 표시 이름을 네오상사에서 호이상사로 변경했습니다.',
      'V2 전환 준비실 영문 표기를 HOI COMPANY로 변경했습니다.',
      '기존 유저 데이터와 연결된 카드 ID, 휘장 ID, 로컬 저장소 키는 호환성을 위해 유지합니다.'
    ]
  },
  {
    id: '2026-06-30-v2-account-password-migration',
    time: '2026-06-30 18:40',
    title: 'V2 계정 로그인 정보 이관',
    details: [
      '기존 계정의 bcrypt 비밀번호 해시를 V2 전용 계정 컬렉션에 함께 이관하도록 변경했습니다.',
      '평문 비밀번호는 저장하지 않으며, V2에서도 기존 아이디와 비밀번호로 바로 로그인할 수 있습니다.',
      '인증정보는 일반 게임 데이터 스냅샷과 분리해 관리합니다.'
    ]
  },
  {
    id: '2026-06-30-v2-migration-foundation',
    time: '2026-06-30 18:00',
    title: 'V2 리워크 기반 구축',
    details: [
      'V1 서비스와 데이터를 그대로 유지하면서 /v2에 독립 전환 준비 화면과 V2 코어 구조를 추가했습니다.',
      '제공된 경험치표는 1→2부터 199→200까지 적용하고, 200레벨 경험치 행은 사용하지 않도록 구성했습니다.',
      '기존 레벨은 1900레벨→130레벨 기준 곡선으로 변환하며 레벨당 5의 미사용 스탯 포인트를 지급합니다.',
      'V2 돈과 주식은 0으로 시작하고, 비밀번호를 제외한 카드·장비·회사·인벤토리 원본은 별도 스냅샷으로 보존합니다.',
      '별도 테스트 DB에서 V2를 실행할 수 있는 테스트 서버 설정과 이관 설계표를 추가했습니다.'
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      '모험의 남은 경험치 비율 보상은 50레벨까지 기존과 동일하게 적용하고, 51레벨부터 레벨당 0.5%씩 누적 감쇠되도록 조정했습니다.',
      '증강 2대2에서 예약 행동이 즉시 스킵처럼 넘어가지 않도록 현재 행동자의 턴을 잠시 보여준 뒤 실행되게 조정했습니다.'
    ]
  },
  {
    id: '2026-06-09-pvp-augment-hp-stack-shield',
    time: '2026-06-09 20:10',
    title: '증강 2대2 전투 템포 조정',
    items: [
      '증강 2대2의 기본 HP를 300에서 200으로 낮춰 킬이 더 잘 나오도록 조정했습니다.',
      '기본공격 명중 시 스택을 쌓아 추가 피해를 주는 증강, 기본공격 회복/튕김/쿨타임 감소 증강, 카드 바꿔치기 증강을 추가했습니다.',
      '증강 전투의 임시 실드는 받은 유저의 다음 행동 주기까지 유지된 뒤 사라지도록 만료 처리를 정리했습니다.'
    ]
  },
  {
