...
1. 테스트 PC에 `0.4.0`을 설치합니다.
2. 작은 수정과 함께 `package.json`을 `0.4.1`로 올립니다.
3. 앞 단계와 동일하게 테스트하고 `tcg-v0.4.1` 태그를 배포합니다.
4. 테스트 PC에서 0.4.0 앱을 완전히 종료했다가 다시 실행합니다.
5. 앱이 업데이트를 확인하고 0.4.1을 내려받는지 확인합니다.
...
앱은 더 낮은 버전으로 자동 복귀하지 않도록 설정되어 있습니다. `0.4.0`에 문제가 있다면 `0.3.0`을 다시 최신으로 올리지 말고 수정한 `0.4.1`을 최대한 빨리 배포합니다. 이를 롤포워드 복구라고 합니다.
