...
앱 버전의 기준은 `tcg-desktop/package.json`의 `version`입니다. 현재 첫 계정 지원 배포 버전은 `0.3.0`입니다.

이후 수정 배포는 다음처럼 항상 더 높은 버전을 사용합니다.

- 버그 수정: `0.3.0` → `0.3.1`
- 호환되는 기능 추가: `0.3.1` → `0.4.0`
...
```text
Hoi-Card-Desk-0.3.0-x64.exe
Hoi-Card-Desk-0.3.0-x64.exe.blockmap
latest.yml
```

`latest.yml`의 `version`도 `0.3.0`이어야 합니다. 로컬 `release` 폴더는 확인용이며 Git에 추가하지 않습니다.
...
버전이 `0.3.0`이면 만들어지는 태그는 정확히 `tcg-v0.3.0`이어야 합니다. `v0.3.0`, `tcg-0.3.0`, `tcg-v0.3`은 사용하지 않습니다.
...
Actions가 성공하면 저장소의 **Releases**에서 `tcg-v0.3.0` Release를 엽니다. 다음 세 파일이 모두 있어야 합니다.
...
1. 테스트 PC에 `0.3.0`을 설치합니다.
2. 작은 수정과 함께 `package.json`을 `0.3.1`로 올립니다.
3. 앞 단계와 동일하게 테스트하고 `tcg-v0.3.1` 태그를 배포합니다.
4. 테스트 PC에서 0.3.0 앱을 완전히 종료했다가 다시 실행합니다.
5. 앱이 업데이트를 확인하고 0.3.1을 내려받는지 확인합니다.
...
앱은 더 낮은 버전으로 자동 복귀하지 않도록 설정되어 있습니다. `0.3.0`에 문제가 있다면 `0.2.0`을 다시 최신으로 올리지 말고 수정한 `0.3.1`을 최대한 빨리 배포합니다. 이를 롤포워드 복구라고 합니다.
