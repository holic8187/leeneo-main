...
test('regional shops vary prices within five percent and sell ammunition bundles', () => {
  ...
  assert.ok(peachItems.every((item) => item.itemType === 'potion'));
});
...
test('all throwing-star types sell for one won per unit', () => {
  const character = characterFixture();
  ...
  assert.equal(sale.totalPrice, 7);
  assert.equal(sale.money, 1_007);
