...
test('inventory sorting merges event potions that share a fixed expiration', () => {
  const character = characterFixture();
  const expiresAt = new Date('2026-09-14T00:00:00+09:00');
  character.inventory.items = [
    { stackId: 'cheese-a', itemId: 'late_summer_cheese', quantity: 72, expiresAt },
    { stackId: 'ice', itemId: 'late_summer_shaved_ice', quantity: 15, expiresAt },
    { stackId: 'cheese-b', itemId: 'late_summer_cheese', quantity: 58, expiresAt }
  ];

  sortInventory(character);

  const cheese = buildInventoryView(character).categories.consumable.items
    .filter((item) => item.id === 'late_summer_cheese');
  assert.deepEqual(cheese.map((item) => item.quantity), [100, 30]);
  assert.equal(cheese[0].stackId, 'cheese-a');
  assert.ok(cheese.every((item) => item.expiresAt));
});
