...
test('world contains the Peach Electronics region and hidden transport maps', () => {
  const visibleMaps = WORLD_MAPS.filter((map) => !map.hidden);
  const hiddenMaps = WORLD_MAPS.filter((map) => map.hidden);
  assert.equal(WORLD_MAPS.length, 60);
  assert.equal(visibleMaps.length, 53);
  assert.equal(hiddenMaps.length, 7);
  assert.deepEqual(hiddenMaps.map((map) => map.fieldBossId).filter(Boolean).sort(), [
    'gammam_neo',
    'mad_hwang_manager'
  ]);
  assert.equal(new Set(WORLD_MAPS.map((map) => map.id)).size, 60);
  assert.equal(new Set(WORLD_MAPS.map((map) => map.name)).size, 60);
  assert.deepEqual(
    hiddenMaps.filter((map) => map.transportOnly).map((map) => map.id).sort(),
    [
      'company_bus_in_transit',
      'company_bus_waiting_room',
      'peach_bus_in_transit',
      'peach_bus_waiting_room'
    ]
  );
  assert.equal(WORLD_MAPS.filter((map) => map.region === '피치전자').length, 11);
  ...
  assert.equal(getWorldMap('boss_bald_kim_arena').raidBossId, 'bald_kim_manager');
  assert.deepEqual(getWorldMap('boss_bald_kim_arena').raidExitNpc, {
  ...
  assert.equal(getWorldMap('boss_bald_kim_arena@test-instance').baseMapId, 'boss_bald_kim_arena');
});
...
test('return scroll destinations exclude the company bus stop', () => {
  ...
  assert.equal(findNearestReturnMap('peach_loading_dock').id, 'peach_electronics_lobby');
  assert.equal(findNearestReturnMap('peach_core_chamber').id, 'peach_electronics_lobby');
  assert.notEqual(findNearestReturnMap('peach_core_chamber').id, 'main_lobby');
});

test('regional safe zones expose their own supply shop', () => {
  const safeMaps = WORLD_MAPS.filter((map) => map.safeZone);
  assert.deepEqual(
    safeMaps.map((map) => map.shopId).filter(Boolean).sort(),
    ['headquarters', 'peach_convenience', 'personnel_annex', 'sales_outpost']
  );
  assert.equal(getWorldMap('company_bus_stop').safeZone, true);
  assert.equal(getWorldMap('peach_bus_stop').safeZone, true);
});
...
test('all map connections are valid and bidirectional', () => {
  ...
  for (const map of WORLD_MAPS) {
    if (map.raidBossId || map.transportOnly) continue;
...
test('headquarters maps are reachable by portal and Peach maps are reachable after bus arrival', () => {
  ...
  assert.equal(
    visited.size,
    WORLD_MAPS.filter((map) => (
      !map.raidBossId && !map.transportOnly && map.region !== '피치전자'
    )).length
  );

  const peachVisited = new Set(['peach_electronics_lobby']);
  const peachQueue = ['peach_electronics_lobby'];
  while (peachQueue.length) {
    const current = getWorldMap(peachQueue.shift());
    for (const connection of current.connections) {
      if (peachVisited.has(connection.targetId)) continue;
      peachVisited.add(connection.targetId);
      peachQueue.push(connection.targetId);
    }
  }
  assert.equal(peachVisited.size, 11);
});
...
test('maps expose varied multi-platform layouts with explicit spawn rules', () => {
  ...
  for (const map of WORLD_MAPS) {
    ...
    assert.ok(
      map.safeZone || map.fieldBossId || map.raidBossId || map.transportOnly
        || spawnSlots >= map.layout.maxMonsters,
      `${map.id} does not have enough platform spawn slots`
    );
  }

  const huntingMaps = WORLD_MAPS.filter((map) => !map.safeZone && !map.fieldBossId && !map.raidBossId);
...
test('Peach Electronics provides safe facilities and nine high-level hunting maps', () => {
  const maps = WORLD_MAPS.filter((map) => map.region === '피치전자');
  const lobby = getWorldMap('peach_electronics_lobby');
  const huntingMaps = maps.filter((map) => !map.safeZone);
  assert.equal(lobby.safeZone, true);
  assert.equal(lobby.shopId, 'peach_convenience');
  assert.equal(lobby.layout.id, 'peach_lobby');
  assert.equal(huntingMaps.length, 9);
  assert.ok(huntingMaps.every((map) => map.minLevel >= 130 && map.maxLevel <= 150));
  assert.ok(huntingMaps.every((map) => (map.monsterIds || []).length >= 1));
  assert.ok(new Set(huntingMaps.map((map) => map.layout.id)).size >= 4);
  assert.ok(Math.max(...huntingMaps.map((map) => map.layout.worldWidth)) >= 4_500);
});
