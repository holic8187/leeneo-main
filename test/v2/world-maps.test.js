...
test('world contains forty-two visible maps and three hidden boss maps', () => {
  const visibleMaps = WORLD_MAPS.filter((map) => !map.hidden);
  const hiddenMaps = WORLD_MAPS.filter((map) => map.hidden);
  assert.equal(WORLD_MAPS.length, 45);
  assert.equal(visibleMaps.length, 42);
  assert.equal(hiddenMaps.length, 3);
  assert.deepEqual(hiddenMaps.map((map) => map.fieldBossId).filter(Boolean).sort(), [
    'gammam_neo',
    'mad_hwang_manager'
  ]);
  assert.equal(new Set(WORLD_MAPS.map((map) => map.id)).size, 45);
  assert.equal(new Set(WORLD_MAPS.map((map) => map.name)).size, 45);
...
test('every map is reachable from the main lobby', () => {
  ...
  assert.equal(visited.size, WORLD_MAPS.filter((map) => !map.raidBossId).length);
});
...
test('maps expose varied multi-platform layouts with explicit spawn rules', () => {
  ...
  for (const map of WORLD_MAPS) {
    ...
    assert.ok(
      map.safeZone || map.fieldBossId || map.raidBossId || spawnSlots >= map.layout.maxMonsters,
