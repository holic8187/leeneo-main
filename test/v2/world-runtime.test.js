...
test('title status resistance prevents negative status effects by percentage', () => {
  const player = { combatProfile: { statusResistance: 5 } };
  assert.equal(resistsStatusEffect(player, () => 0.049), true);
  assert.equal(resistsStatusEffect(player, () => 0.05), false);
  assert.equal(resistsStatusEffect({ combatProfile: {} }, () => 0), false);
  assert.equal(
    resistsStatusEffect({ combatProfile: { statusImmunity: 1 } }, () => 0.99),
    true
  );
});
...
test('the displayed evasion stat participates in physical contact dodge', () => {
  const initial = updatePresence({
    userId: 'stat-evasion-user',
    nickname: '회피율 사원',
    mapId: 'newcomer_training',
    x: 0,
    floor: 0,
...
test('simultaneous retaliation from an area attack deals damage only once', () => {
  const mapId = 'peach_loading_dock';
  const initial = updatePresence({
    userId: 'area-retaliation-user',
    nickname: 'area-retaliation-user',
    mapId,
    x: 50,
    floor: 0,
    currentHp: 100_000,
  ...
  const target = initial.monsters.find((entry) => (
    entry.floor === 0 && entry.counterAttackType === 'projectile'
  ));
  const x = target.x < 50 ? target.x + 8 : target.x - 8;
  updatePresence({
    userId: 'area-retaliation-user',
    nickname: 'area-retaliation-user',
    mapId,
  ...
  const attack = useSkillOnMonsters({
    userId: 'area-retaliation-user',
    mapId,
    targetId: target.id,
    baseDamage: 1,
  ...
  const armed = attack.outcomes.filter((outcome) => (
    Number(outcome.monster?.counterAttackReadyAt) > 0
  ));
  assert.ok(armed.length >= 2);
  const readyAt = Math.max(...armed.map((outcome) => outcome.monster.counterAttackReadyAt));
  const retaliationAt = Math.max(readyAt + 1, 3_011);
  const retaliation = updatePresence({
    userId: 'area-retaliation-user',
    nickname: 'area-retaliation-user',
    mapId,
  ...
  const counterEvents = retaliation.contactEvents.filter((event) => (
    ['monster-projectile', 'monster-swing'].includes(event.source)
  ));
...
test('progressive piercing uses the latest caster coordinates supplied with the cast', () => {
  const originalRandom = Math.random;
  let state;
  try {
    Math.random = () => 0.9;
    state = updatePresence({
      userId: 'moving-piercing-user',
      nickname: '이동한 회계 사원',
      mapId: 'newcomer_training',
      x: 8,
      floor: 0,
...
test('decoy summons pull normal monster aggro and receive contact damage first', () => {
  ...
  const monster = initial.monsters.find((entry) => (
    entry.floor === 0 && entry.x >= 5 && entry.x <= 89
  ));
  assert.ok(monster);
...
test('a monster counterattack damages the decoy without also hitting its owner', () => {
  const mapId = 'peach_loading_dock';
  const initial = updatePresence({
    userId: 'counter-decoy-owner',
    nickname: '퍼펫 궁수',
    mapId,
    x: 50,
    floor: 0,
  ...
  const x = monster.x < 50 ? monster.x + 8 : monster.x - 8;
  const facingLeft = x > monster.x;
  const decoySummon = {
    skillId: 'archer-puppet',
    name: '퍼펫',
    icon: 'P',
    role: 'decoy',
    summonHp: 100_000,
  ...
  const decoyHpBeforeCounter = deployed.players[0].decoySummon.summonHp;
  const attack = attackMonster({
    userId: 'counter-decoy-owner',
    mapId,
    monsterId: monster.id,
  ...
  const state = updatePresence({
    ...
    maxMp: 10_000,
    decoySummon,
    now: attack.monster.counterAttackReadyAt + 1
  });
  const counterSummonEvent = state.summonEvents.find((event) => (
    event.monsterId === monster.id
    && ['monster-projectile', 'monster-swing'].includes(event.source)
  ));
  assert.ok(counterSummonEvent);
  assert.ok(counterSummonEvent.damage > 0);
  assert.equal(
    state.players[0].decoySummon.summonHp,
    decoyHpBeforeCounter
      - state.summonEvents.reduce((sum, event) => sum + Number(event.damage || 0), 0)
  );
  assert.equal(counterSummonEvent.destroyed, false);
  assert.equal(state.contactEvents.length, 0);
  assert.equal(state.players[0].currentHp, 100_000);
});
