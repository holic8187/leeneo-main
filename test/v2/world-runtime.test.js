...
test('an occupied map lazily spawns test monsters with configured combat stats', () => {
  ...
  assert.equal(state.monsters.length, 6);
...
test('an occupied compact map replenishes six monsters every eight seconds up to fourteen', () => {
  ...
  assert.equal(heartbeat(1_000).monsters.length, 6);
  assert.equal(heartbeat(8_999).monsters.length, 6);
  assert.equal(heartbeat(9_000).monsters.length, 12);
  assert.equal(heartbeat(17_000).monsters.length, 14);
  assert.equal(heartbeat(25_000).monsters.length, 14);
  assert.equal(heartbeat(33_000).monsters.length, 14);
});
