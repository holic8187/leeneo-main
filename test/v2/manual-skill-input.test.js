...
test('flash jump temporarily owns horizontal movement and ignores stale position replies', () => {
  assert.match(app, /flashJumpMovementPauseUntil: 0/);
  assert.match(app, /physics\.flashJumpMovementPauseUntil = Date\.now\(\) \+ 280/);
  assert.match(app, /const flashJumpPauseMs = Number\(/);
  assert.match(app, /&& !state\.moving\s*&& physics\.jumpSequence === localJumpSequence/);
});

test('automatic combat waits without breaking an active stealth buff', () => {
  assert.match(app, /function isStealthActive\(/);
  assert.match(app, /if \(isStealthActive\(\)\) \{\s*setWorldActivity\('은신 상태 유지 중'\)/);
  assert.match(app, /&& !isStealthActive\(\)\s*&& !getMap\(state\.currentMapId\)\?\.safeZone/);
});

test('Gammam Neo lockout is shown before hidden-street movement begins', () => {
  assert.match(app, /function getMapEntryBlockMessage\(/);
  assert.match(app, /fieldBossLockouts\?\.gammam_neo/);
  assert.match(app, /state\.character\.fieldBossLockouts = \{ \.\.\.data\.fieldBossLockouts \}/);
  assert.match(app, /setWorldActivity\(blockMessage\)/);
  assert.match(routes, /\.find\(\(connectedMap\) => connectedMap && !connectedMap\.hidden\)/);
  assert.match(routes, /fieldBossLockouts: \{ \.\.\.\(profile\.fieldBossLockouts \|\| \{\}\) \}/);
});
