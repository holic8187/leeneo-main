...
test('travel skills use the current map width for a stable pixel distance', () => {
  const warehouseDestination = resolveTravelDestination({
    currentX: 40,
    direction: 1,
    distancePx: 320,
    worldWidth: 2_160
  });
  const compactMapDestination = resolveTravelDestination({
    currentX: 40,
    direction: 1,
    distancePx: 320,
    worldWidth: 760
  ...
  assert.equal(warehouseDestination, 40 + 320 / 2_160 * 100);
  assert.equal(compactMapDestination, 40 + 320 / 760 * 100);
  assert.ok(warehouseDestination < compactMapDestination);
  assert.equal(resolveTravelDestination({
    currentX: 2,
    direction: -1,
    distancePx: 320,
    worldWidth: 760
