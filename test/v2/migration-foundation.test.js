...
test('V2 router exposes the current migration, world, inventory, and shop endpoints', () => {
  ...
  assert.deepEqual(registered, [
    ...
    'POST /api/v2/titles/equip',
    'POST /api/v2/titles/unequip',
    'POST /api/v2/titles/acknowledge',
...
test('workspace login routes retroactively reconcile advancement titles', () => {
  const source = fs.readFileSync(
    path.join(__dirname, '../../src/v2/registerV2Routes.js'),
    'utf8'
  );
  const previewStart = source.indexOf("app.get('/api/v2/migration/preview'");
  const previewEnd = source.indexOf("app.post('/api/v2/migration/prepare'", previewStart);
  const claimStart = source.indexOf("app.post('/api/v2/world/claim-control'");
  const claimEnd = source.indexOf("app.get('/api/v2/ranking'", claimStart);
  const loginStart = source.indexOf("app.post('/api/v2/login'");
  const loginEnd = source.indexOf("app.post('/api/v2/presence/heartbeat'", loginStart);

  assert.equal(
    (source.slice(loginStart, loginEnd).match(/reconcileAdvancementTitles/g) || []).length,
    2
  );
  assert.match(source.slice(previewStart, previewEnd), /reconcileAdvancementTitles\(character\)/);
  assert.match(source.slice(claimStart, claimEnd), /reconcileAdvancementTitles\(current\)/);
});
