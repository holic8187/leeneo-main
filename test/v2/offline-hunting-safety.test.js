...
test('background handoff resets the offline combat baseline before the first sweep', () => {
  const character = characterFixture();
  const handoff = prepareOfflineHuntingHandoff(character, 20_000);

  assert.equal(character.huntingTime.remainingSeconds, 90);
  assert.equal(new Date(handoff.baselineAt).getTime(), 20_000);
  assert.equal(handoff.safetyStopped, false);
  assert.deepEqual(getOfflineHuntingActionTimes({
    elapsedMs: 500,
    intervalMs: 1_200,
    now: 20_500
