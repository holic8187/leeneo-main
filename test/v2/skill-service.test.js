...
test('six-step repair resolves as six separate hits', () => {
  const skill = SKILL_DEFINITIONS.extended_de980f68ec;
  const values = resolveSkillValues(skill, skill.maxLevel);

  assert.equal(skill.name, '6단 수리');
  assert.equal(values.hits, 6);
  assert.equal(resolveSkillCastProfile(values).hitCount, 6);
});

test('zero error marks normal monsters for fixed execute damage', () => {
  const skill = SKILL_DEFINITIONS.extended_e9c47b999a;
  const values = resolveSkillValues(skill, skill.maxLevel);

  assert.equal(skill.name, '오차 0원');
  assert.equal(values.instantKillNormalMonster, true);
  assert.match(skill.description, /199,999/);
});
...
test('reviewed utility cooldowns and formerly missing active effects are runtime-ready', () => {
  const byName = (name) => Object.values(SKILL_DEFINITIONS).find(
    (definition) => definition.name === name
  );
  const wakeUp = byName('정신 차려!');
  assert.equal(resolveSkillValues(wakeUp, 1).cooldownSeconds, 600);
  assert.equal(resolveSkillValues(wakeUp, wakeUp.maxLevel).cooldownSeconds, 300);
  ...
  for (const name of [
    ...
    assert.equal(resolveSkillValues(definition, definition.maxLevel).cooldownSeconds, 600);
    assert.match(definition.description, /쿨타임 10분/);
  }

  assert.equal(byName('계약금 회수').effect, 'damage');
  assert.equal(byName('업무 방해').effect, 'monster-debuff');
  assert.equal(byName('오류 반환').effect, 'magic-reflect');
  assert.equal(byName('비상 연막').effect, 'smoke-zone');
  assert.equal(byName('고충 처리').effect, 'party-cleanse-dispel');
  assert.equal(byName('개박살').effect, 'monster-dispel');

  const workInterference = resolveSkillValues(byName('업무 방해'), 20);
  assert.equal(workInterference.enemyAttackReduction, 20);
  assert.equal(workInterference.enemyPhysicalDefenseReduction, 20);
  assert.equal(resolveSkillValues(byName('연쇄 방전'), 30).chainFromPrimaryTarget, true);
  assert.equal(resolveSkillValues(byName('전략 지원선'), 30).pathPartyHealPercent, 20);
  const emergencyRepair = resolveSkillValues(byName('응급 정비'), 30);
  assert.equal(emergencyRepair.castDamageReductionPercent, 30);
  assert.equal(emergencyRepair.castProtectionSeconds, 1);
});
