...
test('return scrolls use the visible map and authoritative destination coordinates', () => {
  ...
  assert.match(routes, /findNearestReturnMap\(returnScrollSourceMapId\)/);
  assert.match(app, /JSON\.stringify\(\{ itemId, mapId: state\.currentMapId \}\)/);
  assert.match(app, /function applyReturnScrollMap/);
  assert.match(app, /renderWorldMap\(data\.map\.id, 0, x, floor\)/);
});
